...
以降のチュートリアルでは、このチュートリアルで作成したDBシステムをそのまま使用します。継続してPostgreSQLチュートリアルを進める場合は、DBシステムを削除しないでください。

次のチュートリアルをすぐに実施しない場合は、コストを抑えるためにDBシステムを停止することを推奨します。停止したDBシステムは、作業を再開する前に起動します。今後使用しない場合は、DBシステムを削除します。

停止、起動、削除の詳細は、[OCI Database with PostgreSQLデータベース・システムの停止および起動](https://docs.oracle.com/ja-jp/iaas/Content/postgresql/stop-start-db.htm)も参照してください。
...
3. **他のアクション** → **停止** をクリックします。

4. 確認ダイアログの内容を確認し、**停止** をクリックします。

5. DBシステムのステータスが停止中になり、その後停止済みになることを確認します。

停止中または停止済みのDBシステムには接続できません。以降のチュートリアルを再開する前に、次の手順でDBシステムを起動してください。

## 6-2. DBシステムを起動する

1. コンソールメニューから **データベース** → **PostgreSQL** → **DBシステム** を選択します。

2. 停止済みのDBシステムをクリックします。ここでは `TestPostgreSQL` を選択します。

3. **他のアクション** → **起動** をクリックします。

4. 確認ダイアログの内容を確認し、**起動** をクリックします。

5. DBシステムのステータスが起動中になり、その後アクティブになることを確認します。

DBシステムがアクティブになった後、以降のチュートリアルの作業を再開します。

## 6-3. DBシステムを削除する

このチュートリアルで作成したDBシステムを今後使用しない場合は、削除します。
