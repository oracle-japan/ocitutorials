...
この手順は、構築手法に **[リソース・マネージャ](../#5-2-リソースマネージャ)** を使用するか **[Terraform](../#5-12-terraform)** CLIを使用するかで異なります。

1. **[リソース・マネージャを使用する場合](#1-1-リソースマネージャを使用する場合)**
    - **[構成ソース・プロバイダ](../#5-14-構成ソースプロバイダ)** 作成
    - **[スタック](../#5-3-スタック)** 作成
2. **[TerraformCLIを使用する場合](#1-2-terraform-cliを使用する場合)**
    - **Terraform** 実行環境構築
    - **Terraform** スクリプト作成

以降では、2つの異なる構築手法毎にその手順を解説します。

## 1-1. リソース・マネージャを使用する場合
...
※3） 例えば **x9-ol905** と指定した場合、計算ノードのホスト名は **inst-xxxxx-x9-ol905** となります。（ **xxxxx** はランダムな文字列）  
...
この手順は、構築手法に **[リソース・マネージャ](../#5-2-リソースマネージャ)** を使用するか **Terraform** CLIを使用するかで異なり、以降では2つの異なる構築手法毎にその手順を解説します。

## 2-1. リソース・マネージャを使用する場合
...
この手順は、構築手法に **[リソース・マネージャ](../#5-2-リソースマネージャ)** を使用するか **[Terraform](../#5-12-terraform)** CLIを使用するかで異なり、以降では2つの異なる構築手法毎にその手順を解説します。

## 5-1. リソース・マネージャを使用する場合
