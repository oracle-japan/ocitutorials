...
※2）アプリケーション実行時の環境変数で利用するハードウェアカウンタを指定します。これに対し **Low Level API** は、APIコール時に利用するハードウェアカウンタを指定するため、これを変更する場合再コンパイルが必要です。  
...
利用可能なハードウェアカウンタ一覧は、 **[2-2. PAPIインストール](#2-2-papiインストール)** の手順の中で取得することが可能です。
...
なお設定値 **PAPI_FP_** は、倍精度・単精度の何れのイベントも含むため、これらを明示的に区別する必要がある場合は **PAPI_DP_** （倍精度）や **PAPI_SP_** （単精度）を使用します。
...
この出力より、実時間 **33.01秒** で浮動小数点演算を **54.0 G回** 実行し、 **1.6 GFLOPS** の性能であることがわかります。
