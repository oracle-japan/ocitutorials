...
※5）以降では、 **NPB** の **FT Class D** の出力の **Time in seconds** を所要時間として扱います。
...
次に、以下コマンドを計算ノードのプロファイリング利用ユーザで実行し、 **asis** の所要時間を計測します。
...
次に、以下コマンドを計算ノードのプロファイリング利用ユーザで実行し、プロファイリング取得時の所要時間を計測します。  
...
以上より、 **asis** とプロファイリング取得時の所要時間に大きな差が無い（32.41秒と32.74秒）ことが確認できました。  
...
この結果から、 **asis** とチューニング適用時所要時間を比較し、チューニングの効果を確認します。
