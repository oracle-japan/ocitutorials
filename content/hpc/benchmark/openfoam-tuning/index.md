---
title: "CFD解析フローのトータルコストパフォーマンを向上させるOpenFOAM関連Tips"
description: "OpenFOAMは、CAE分野で多くの利用実績を持つオープンソースのCFDアプリケーションで、計算時に多くのメモリ帯域を使用したり実行中に多くのデータをファイルシステムに書き出したりする特性があるため、これらを考慮した実行方法を採用することでその性能を大きく向上させることが可能です。本パフォーマンス関連Tipsは、Intel Ice Lakeプロセッサを搭載するベアメタルシェイプBM.Optimized3.36をクラスタ・ネットワークでノード間接続するHPCクラスタでOpenFOAMを使用する際、CFD解析フローをコストパフォーマンス良く実行するという観点で有益なTipsを解説します。"
weight: "2204"
tags:
- hpc
params:
...
本パフォーマンス関連Tipsは、 **Intel Ice Lake** プロセッサを搭載するベアメタルシェイプ **[BM.Optimized3.36](https://docs.oracle.com/ja-jp/iaas/Content/Compute/References/computeshapes.htm#bm-hpc-optimized)** を **[クラスタ・ネットワーク](../../#5-1-クラスタネットワーク)** でノード間接続するHPCクラスタで **[OpenFOAM](https://www.openfoam.com/)** を実行する際、CFD解析フロー全体に跨るトータルコストパフォーマンスを向上させるための考慮点を解説します。  
この際、以下4点のコストパフォーマンスに影響するTipsをこの順に検証し、最終的にマルチノード（本パフォーマンス関連Tipsでは最大8ノードで実行しています。）で **OpenFOAM** を実行する際のコストパフォーマンスを最適化する実行方法にたどり着くようアプローチします。

1. **[ストレージ領域を考慮した最適な実行方法](#1-ストレージ領域を考慮した最適な実行方法)**
2. **[最適なノード内並列実行方法](#2-最適なノード内並列実行方法)**
3. **[スケーラビリティーを考慮した最適なノード間並列実行方法](#3-スケーラビリティーを考慮した最適なノード間並列実行方法)**
4. **[コストパフォーマンス最大となるノード数の検証](#4-コストパフォーマンス最大となるノード数の検証)**
...
本パフォーマンス関連Tipsは、以下の環境・条件で検証を実施しています。
...
- **OpenFOAM**
    ...
    - 解析対象モデル
        ...
        - Mesh stats（ **HPC_Motorbike** のデフォルト）
            - points： 26426
            - faces： 65863
            - internal faces： 54137
            - cells： 20000
        - 解析対象タイムステップ： 0 - 500（ **HPC_Motorbike** のデフォルト）
...
以降の検証により、以下の実行方法で解析対象モデルのCFD解析フローのコストパフォーマンスが最大となることが判明しています。

- CFD解析フローのストレージ領域にNVMe SSDローカルディスクを使用せずファイル共有ストレージ（※6）を使用
- **NPS2** で作成した計算ノードを **8** ノード使用
- NUMAノード単位のサイクリック分割でノード当たり **36** プロセスを割り当てる
- **UCX_RNDV_THRESH** に **intra:16kb,inter:auto** を使用

※6）より性能の高いファイル共有ストレージを採用することで更なる優位性の向上が期待できますが、ファイル共有ストレージの性能向上は **[OCI HPCテクニカルTips集](../../#3-oci-hpcテクニカルtips集)** の **[HPC/GPUクラスタ向けファイル共有ストレージの最適な構築手法](../../tech-knowhow/howto-configure-sharedstorage/)** が参考になります。なお、本パフォーマンス関連Tipsで使用しているファイル共有ストレージは、このドキュメント中最も性能の低い **標準FSS** です。

この検証結果は、解析対象モデルが異なる場合そのまま適用できないことになりますが、本パフォーマンス関連Tipsで採用している検証手法を参考に、自身の解析対象モデルに沿った結論を導き出すことが可能です。
...
1. **decomposePar** による並列計算用メッシュ分割（以降" **ステップ 1.** "と呼称します。）（※6）
2. **potentialFoam** ・ **simpleFoam** によるソルバー実行（以降" **ステップ 2.** "と呼称します。）（※6）
3. **reconstructParMesh** ・ **reconstructPar** による並列計算結果マージ処理（以降" **ステップ 3.** "と呼称します。）

※6） **Allrun** スクリプト内から実行されます。
...
1. **ステップ 1.**
    - **L** の場合
        1. メッシュ分割前のケースディレクトリ（※7）を **S** からヘッドノードの **L** にコピー（ **T1** ）
...
2. **ステップ 2.**
    - **L** の場合
        - **ステップ 1.** が **L** の場合
            1. ヘッドノードの **L** 上のメッシュ分割後のケースディレクトリ（※8）をその他計算ノードの **L** にコピー（※9）（ **T3** ）
            2. **L** 上で **potentialFoam** を実行（ **T4** ）
            3. **L** 上で **simpleFoam** を実行（ **T5** ）
        - **ステップ 1.** が **S** 場合
            1. **S** 上のメッシュ分割後のケースディレクトリ（※8）を全計算ノードの **L** にコピー（※10）（ **T6** ）
            2. **L** 上で **potentialFoam** を実行（ **T4** ）
            3. **L** 上で **simpleFoam** を実行（ **T5** ）
...
3. **ステップ 3.**
    - **L** の場合（ **ステップ 2.** は **L** で実行することになります。）
        1. その他計算ノードの **L** 上のケースディレクトリをヘッドノードの **L** にコピー（※11）（ **T9** ）
        2. ヘッドノードの **L** 上で **reconstructParMesh** を実行（ **T10** ）
        3. ヘッドノードの **L** 上で **reconstructPar** を実行（ **T11** ）
        4. ヘッドノードの **L** 上のマージ処理後の **processorxx** ディレクトリを除くケースディレクトリ（※12）を **S** にコピー（ **T12** ）
    - **S** の場合
        1. 全計算ノードの **L** 上のケースディレクトリ（※13）を **S** にコピー（※14）（ **T13** ）（ **ステップ 2.** が **S** の場合必要ありません。）
        2. VMの **S** 上で **reconstructParMesh** ・ **reconstructPar** を実行

ここで **Tx** は、その工程の所要時間を表し、以降の解説で引用します。

※7）総ファイル容量が1,793 MB、総ファイル・ディレクトリ数が63です。  
※8）288並列の場合、総ファイル容量が3,886 MB、総ファイル・ディレクトリ数が6,688です。  
※9）このコピーは、ヘッドノードから起動する **[pdsh](https://github.com/chaos/pdsh)** でその他計算ノードからrsyncを並列実行し、所要時間の短縮を行っています。    
※10）このコピーは、ヘッドノードから起動する **pdsh** で全計算ノードからrsyncを並列実行し、所要時間の短縮を行っています。  
※11）このコピーは、ヘッドノードから起動する **pdsh** でその他計算ノードからrsyncを並列実行し、コピー対象を自身の計算結果を格納する **processorxx** ディレクトリのみに限定することで、所要時間の短縮を行っています。  
※12） **writeInterval** 値が **10** の場合、総ファイル容量が71,309 MB、総ファイル・ディレクトリ数が666です。  
※13）288並列で **writeInterval** 値が **10** の場合、コピー後の **S** 上のケースディレクトリの総ファイル容量が94,328 MB、総ファイル・ディレクトリ数が179,776です。  
※14）このコピーは、ヘッドノードから起動する **pdsh** で全計算ノードからrsyncを並列実行し、コピー対象を自身の計算結果を格納する **processorxx** ディレクトリのみに限定することで、所要時間の短縮を行っています。
...
以降の検証結果から、 <b><u>以下の実行方法でトータルコストパフォーマンスが最大となる</u></b> ことが判明しています。
...
- **UCX_RNDV_THRESH** ： **auto** / **16kb** / **128kb** （※16）
- **coll_hcoll_enable** ： **0** / **1** （※17）
- **coll_ucc_enable** ： **0** / **1** （※18）

※15）NUMAノードに対するMPIプロセスの分割方法で、詳細は **[OCI HPCパフォーマンス関連情報](../../#2-oci-hpcパフォーマンス関連情報)** の **[パフォーマンスを考慮したプロセス・スレッドのコア割当て指定方法（BM.Optimized3.36編）](../../benchmark/cpu-binding/)** を参照してください。  
※16）詳細は **[OCI HPCパフォーマンス関連情報](../../#2-oci-hpcパフォーマンス関連情報)** の **[OpenMPIのMPI通信性能に影響するパラメータとその関連Tips](../../benchmark/openmpi-perftips/)** の **[3-6. UCX_RNDV_THRESH](../../benchmark/openmpi-perftips/#3-6-ucx_rndv_thresh)** を参照してください。  
※17）詳細は **[OCI HPCパフォーマンス関連情報](../../#2-oci-hpcパフォーマンス関連情報)** の **[OpenMPIのMPI通信性能に影響するパラメータとその関連Tips](../../benchmark/openmpi-perftips/)** の **[3-1. coll_hcoll_enable](../../benchmark/openmpi-perftips/#3-1-coll_hcoll_enable)** を参照してください。  
※18）詳細は **[OCI HPCパフォーマンス関連情報](../../#2-oci-hpcパフォーマンス関連情報)** の **[OpenMPIのMPI通信性能に影響するパラメータとその関連Tips](../../benchmark/openmpi-perftips/)** の **[3-9. coll_ucc_enable](../../benchmark/openmpi-perftips/#3-9-coll_ucc_enable)** を参照してください。  
...
以降の検証結果から、 <b><u>以下の実行時パラメータでトータルコストパフォーマンスが最大となる</u><b> ことが判明しています。
...
- **UCX_TLS** ： **all** / **self,sm,rc** / **self,sm,ud** （※19）
- **UCX_ZCOPY_THRESH** ： **auto** / **128kb** （※20）
- **UCX_RNDV_THRESH** ： **inter:auto** / **inter:128kb**

※19）詳細は **[OCI HPCパフォーマンス関連情報](../../#2-oci-hpcパフォーマンス関連情報)** の **[OpenMPIのMPI通信性能に影響するパラメータとその関連Tips](../../benchmark/openmpi-perftips/)** の **[3-4. UCX_TLS](../../benchmark/openmpi-perftips/#3-4-ucx_tls)** を参照してください。なお、 **self,sm,ud** は4ノードと8ノードにのみ使用します。  
※20）詳細は **[OCI HPCパフォーマンス関連情報](../../#2-oci-hpcパフォーマンス関連情報)** の **[OpenMPIのMPI通信性能に影響するパラメータとその関連Tips](../../benchmark/openmpi-perftips/)** の **[3-7. UCX_ZCOPY_THRESH](../../benchmark/openmpi-perftips/#3-7-ucx_zcopy_thresh)** を参照してください。   
...
- **[Test inter1](#3-1-test-inter1)**
    ...
    - ノード当たりプロセス数は **[Test intra1](#2-1-test-intra1)** で決定した最適値を使用（※21）
    - 以下実行時パラメータは **[Test intra2](#2-2-test-intra2)** で決定した最適値を使用
        - **NPS**
        - プロセス分割方法
...
※21） **[Test intra1](#2-1-test-intra1)** ではプロセス数 **36** が最も性能が良いという結果ですが、プロセス数 **32** との性能差が **3.6 %** と小さくノード間並列に拡張する際に性能が逆転する可能性に配慮し、両者を評価対象とします。
...
# 4. コストパフォーマンス最大となるノード数の検証

本章は、これまでの検証結果を元に、コストパフォーマンスが最大となるノード数を検証します。
...
以上より、8ノードまでのHPCクラスタで本解析モデルを実行する場合の <b><u>コストパフォーマンス最大となるノード数は **8** ノード</u></b> であることがわかります。
