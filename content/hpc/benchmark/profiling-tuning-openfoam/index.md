...
以上より、 **MPI_Allreduce** と **Exchange** 型通信に対する最適なパラメーター設定が異なりますが、これらのパラメータは実行時に一意に指定する必要があり、通信タイプ毎に異なるパラメータを使用することが出来ないため、2個のホットスポットに対して総合的に最も効果のあるパラメータを見つけ出す必要があります。  
そこで、これら通信タイプ毎の最適なパラメータ設定をたすき掛けして作成される以下8通りのパラメータの組み合わせを抽出し、この中から最も性能の良いパラメータを導きます。
