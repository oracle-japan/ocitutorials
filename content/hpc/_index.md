...
HPC/機械学習ワークロード向けシェイプに搭載されるNVMe SSDローカルディスクは、アプリケーション実行時の高速スクラッチ領域として利用することを想定しており、有効に活用することで所要時間の短縮が可能です。ただこの場合、計算開始時と終了時にファイル共有ストレージとの間で発生するデータコピーや、計算途中に計算/GPUノード間のNVMe SSDローカルディスクで発生するデータコピーに於いて、NVMe SSDローカルディスクが持つ高いIOPS性能を引き出す工夫が必要です。  
