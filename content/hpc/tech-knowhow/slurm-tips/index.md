...
構築する **Slurm** 環境は、計算ノードに3ノードの **[BM.Optimized3.36](https://docs.oracle.com/ja-jp/iaas/Content/Compute/References/computeshapes.htm#bm-hpc-optimized)** （搭載コア数： 36、 搭載メモリ量： 512 GB（**Slurm** 設定上の利用可能メモリ量を500,400 MBに設定））を使用し、以下のパーティション構成とします。
...
- パーティション **small** に投入されたジョブは実行中ジョブの総使用コア数と総使用メモリ量がそれぞれ36コアと500,400 MBを超えない範囲で先着順にノード共有実行
