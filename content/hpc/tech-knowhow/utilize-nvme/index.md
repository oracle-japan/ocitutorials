...
description: "HPCワークロード向けシェイプに搭載されるNVMe SSDローカルディスクは、アプリケーション実行中のファイルシステムへのアクセスを高速化することを想定して搭載されており、有効に活用することでアプリケーション実行時間の短縮が可能です。ただこの場合、計算処理前後にファイル共有ストレージとの間で発生するデータコピーや、各ノードのNVMe SSDローカルディスク間で発生するデータコピーに於いて、NVMe SSDローカルディスクが持つ高い性能を引き出す工夫が必要です。本テクニカルTipsは、NVMe SSDローカルディスクを有効活用することでアプリケーション実行時間を短縮し、シミュレーションコスト削減を実現するための手法を解説します。"
weight: "3206"
tags:
- hpc
params:
...
次に、ストレージ領域に **L** を使用しない場合、前述の **1.** と **3.** のステップが不要となり、以下の1ステップでアプリケーションが実行されます。
...
3. ヘッドノードでメッシュ分割を行う場合のメッシュ分割後入力データを <u>ヘッドノード **L** → その他ノード **L**</u> にコピー
4. ヘッドノードでマージ処理を行う場合のマージ処理前出力データを <u>その他ノード **L** → ヘッドノード **L**</u> にコピー（※3）

※2）このデータコピーは、同一ディレクトリに対して複数の計算/GPUノードがrsyncで同期することによる競合を避けるため、それぞれの計算/GPUノードが自身の持つマージ処理前出力データのみをその対象とする必要があり、これを実現するスクリプトを用意して **pdsh** から起動します。以下スクリプトは、 **OpenFOAM** での利用を想定してこのデータコピーを実現するスクリプトで、各MPIプロセスが **prosessorx** ディレクトリ以下に自身の出力データを格納するため、この配下のみをコピーします。
...
※3）このデータコピーは、同一ディレクトリに対して複数のその他ノードがrsyncで同期することによる競合を避けるため、それぞれの計算/GPUノードが自身の持つマージ処理前出力データのみをその対象とする必要があり、これを実現するスクリプトを用意して **pdsh** から起動します。

![Parallel copy](parallel_copy.png)

この並列データコピーは、 **[Slurm](https://slurm.schedmd.com/)** 環境でバッチジョブとして実行する場合、以下のような記述をジョブスクリプトに指定して実現します。
...
- **L** を利用する
    ...
    3. メッシュ分割後の入力データをヘッドノードの **L** からその他ノードの **L** にコピー（ **T3** ）
...
- **L** を利用する
    1. **L** を利用して計算処理を実行（ **T5** ）
    2. マージ処理前の出力データをその他ノードの **L** からヘッドノードの **L** にコピー（ **T6** ）
