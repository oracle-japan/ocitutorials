...
ここでは、 **[クラスタ・ネットワーク](../../#5-1-クラスタネットワーク)** 対応シェイプのインスタンス作成時デフォルトの **OCA** プラグインの状態に於いて、HPC関連の2個のプラグインのみが有効となるように **OCI CLI** コマンドを実行しています。
...
1. **OCI** コンソールを使用する方法  
この方法は、 **OCI** コンソールの **インスタンス** 画面からコピーする計算/GPUノードのインスタンス名や **OCID** を使用し、エクセルを介してホストリストを作成する方法です。  
