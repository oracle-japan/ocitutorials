...
1. **[カスタム・イメージ取得用HPC/GPUクラスタ構築](#1-カスタムイメージ取得用hpcgpuクラスタ構築)**
2. **[クラスタ・ネットワーク接続用ソフトウェアインストール](#2-クラスタネットワーク接続用ソフトウェアインストール)**
3. **[クラスタ・ネットワーク接続・確認](#3-クラスタネットワーク接続確認)**
4. **[カスタム・イメージ取得](#4-カスタムイメージ取得)**
5. **[cloud-config作成](#5-cloud-config作成)**
6. **[インスタンス構成作成](#6-インスタンス構成作成)**
7. **[クラスタ・ネットワーク作成](#7-クラスタネットワーク作成)**
...
1. cloud-config適用除外  
本チュートリアルで作成している **[cloud-init](../../#5-11-cloud-init)** 設定ファイルのcloud-configは、その処理内容をカスタム・イメージ取得用の計算ノードに適用する必要が無いため、作成しません。  
またこれに伴い、後に作成する **[インスタンス構成](../../#5-7-インスタンス構成)** で指定しているcloud-configの適用も、実施しません。

2. イメージ変更  
**インスタンス構成** 作成時に指定するイメージは、 使用するOSが **Rocky Linux** 、 **CentOS** 、又は **Ubuntu** かに合わせて、以下を指定します。

- Rocky Linux  
**イメージの変更** ボタンをクリックして表示される以下 **Select an image** サイドバーで **Rocky Linux** を選択し表示される **Rocky Linux 8.7 - Free(x86_64)** を選択して表示される **イメージ・ビルド** フィールドで **8.7.3** （選択できなければ最新を選択）を選択し **イメージの選択** ボタンをクリックします。

  ![画面ショット](console_page01.png)  
  ![画面ショット](console_page01-2.png)

- CentOS  
**イメージの変更** ボタンをクリックして表示される以下 **イメージの選択** サイドバーで **CentOS** を選択し表示される **CentOS 7** を選択して表示される **イメージ・ビルド** フィールドで **2023.04.20-0** （選択できなければ最新を選択）を選択し **イメージの選択** ボタンをクリックします。

  ![画面ショット](console_page02.png)

- Ubuntu  
**イメージの変更** ボタンをクリックして表示される以下 **イメージの選択** サイドバーで **My Images** を選択し表示される **イメージOCID** ボタンを選択して表示される **イメージOCID** フィールドに **Canonical-Ubuntu-24.04-2025.05.20-0** のOCIDを **[ここ](https://docs.oracle.com/en-us/iaas/images/ubuntu-2404/canonical-ubuntu-24-04-2025-05-20-0.htm)** のページから自身の使用する **リージョン** に合わせて入力して **イメージの選択** ボタンをクリックします。

  ![画面ショット](console_page02-1.png)

3. **OCA** 有効化のスキップ  
**インスタンス構成** 作成時に有効化している **OCA** の **Compute HPC RDMA Auto-Configuration** と **Compute HPC RDMA Authentication** のプラグインは、 **Rocky Linux** と **CentOS** では **OCA** を使用せず、 **Ubuntu** ではのちの手順で有効化を行うため、何れのOSの場合もここでは有効化を行いません。

# 2. クラスタ・ネットワーク接続用ソフトウェアインストール

## 2-0. 概要

本章は、 **[クラスタ・ネットワーク](../../#5-1-クラスタネットワーク)** に接続するためのソフトウェアのインストールを、 **Rocky Linux** 8.7、 **CentOS** 7.9、及び **Ubuntu** 24.04に分けて解説します。  
以下は、その手順概要です。
...
### 2-3-1. カスタム・イメージ取得用計算/GPUノードログインと事前準備

**[1. カスタムイメージ取得用HPC/GPUクラスタ構築](#1-カスタムイメージ取得用hpcgpuクラスタ構築)** で参照したチュートリアルの手順に従い、計算/GPUノードにログインします。  
この際 **Ubuntu** のインストール時に作成されるログインユーザが **ubuntu** であることを考慮し、以下のようにBastionノードからubuntuユーザでSSHログインします。
...
```sh
$ export MPIROOT=/usr/mpi/gcc/openmpi-4.1.9a1
$ export PATH=${MPIROOT}/bin:${PATH}
$ export LD_LIBRARY_PATH=${MPIROOT}/lib:${LD_LIBRARY_PATH}
$ for hname in `cat ~/hostlist.txt`; do ssh -oStrictHostKeyChecking=accept-new $hname : > /dev/null 2>&1; done
$ mpirun -n 2 -N 1 -hostfile ~/hostlist.txt -x UCX_NET_DEVICES=mlx5_2:1 /usr/mpi/gcc/openmpi-4.1.9a1/tests/imb/IMB-MPI1 -msglog 27:28 pingpong
#------------------------------------------------------------
#    Intel (R) MPI Benchmarks 2018, MPI-1 part    
#------------------------------------------------------------
# Date                  : Thu Apr 23 04:59:46 2026
# Machine               : x86_64
# System                : Linux
# Release               : 6.8.0-1026-oracle
# Version               : #27-Ubuntu SMP Thu Apr 24 15:44:52 UTC 2025
# MPI Version           : 3.1
...
# /usr/mpi/gcc/openmpi-4.1.9a1/tests/imb/IMB-MPI1 -msglog 27:28 pingpong
...
#---------------------------------------------------
       #bytes #repetitions      t[usec]   Mbytes/sec
            0         1000         2.13         0.00
    134217728            1     11098.08     12093.78
    268435456            1     22223.74     12078.77
...
本章は、 **[カスタム・イメージ](../../#5-6-カスタムイメージ)** 取得用2ノードHPC/GPUクラスタの計算/GPUノードのどちらか一台で、以下の手順に従い **カスタム・イメージ** を取得します。
...
**[カスタム・イメージ](../../#5-6-カスタムイメージ)** を使用する **[クラスタ・ネットワーク](../../#5-1-クラスタネットワーク)** の作成は、 **クラスタ・ネットワーク** 用ネットワークインターフェースが **カスタム・イメージ** 取得インスタンス用の静的IPアドレスを含むため、そのままでは全てのインスタンスに同じIPアドレスが設定され、IPアドレス競合により **クラスタ・ネットワーク** に接続することが出来ません。

そこで、 **カスタム・イメージ** 取得前に **クラスタ・ネットワーク** 用ネットワークインターフェース設定を削除しますが、この方法は使用するOSやシェイプにより異なるため、以降ではその組み合わせ毎に手順を解説します。
