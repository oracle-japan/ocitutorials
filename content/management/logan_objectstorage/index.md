...
Log Analyticsは、管理エージェントやコネクタ・ハブ経由のログ収集だけでなく、オブジェクト・ストレージに格納されたデータについてもログとして収集することができます。例えば、Monitoringのメトリックデータは、標準的な構成ではLog Analyticsへ取り込むことができませんが、オブジェクト・ストレージを経由することで収集が可能となり、CPUやメモリ使用率などのリソース情報と各種ログを同じダッシュボードに統合することが可能になります。
