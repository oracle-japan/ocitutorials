...
- スタティック/オンデマンドクラスタ
   ...
   なおオンデマンドクラスタは、 **[Cluster Autoscaling](#5-9-cluster-autoscaling)** が実現します。
...
現時点は、 **[マーケットプレース](#5-5-マーケットプレイス)** から無料で利用可能な **[スタック](#5-3-スタック)** を利用してブロック・ボリュームとベアメタルインスタンスからNFSでファイル共有ストレージ環境を自動構築するチュートリアル **[ブロック・ボリュームでNFSファイルサーバを構築する](/ocitutorials/hpc/spinup-nfs-server/)** を利用することが可能です。

## 1-4. チュートリアルを組み合わせた実践的HPCシステム構築

本章は、異なるカテゴリのチュートリアルを組み合わせてより実践的なHPCシステムを構築する手法を紹介します。

近日公開予定のチュートリアルは、 **[ブロックストレージでNFSファイルサーバを構築する](/ocitutorials/hpc/spinup-nfs-server/)** で構築したファイル共有ストレージを **[HPCクラスタ](#1-1-hpcクラスタ)** のカテゴリや **[機械学習環境](#1-2-機械学習環境)** のカテゴリのチュートリアルで構築するHPCクラスタやGPUクラスタの共有ストレージ領域として構築するものです。公開まで、暫くお待ちください。
...
- **[ベアメタルGPUインスタンスのクラスタ・ネットワーク接続用ネットワークインターフェース作成方法](/ocitutorials/hpc/tech-knowhow/gpu-rdma-interface/)**
   ...
   本テクニカルTipsは、このインタフェース作成方法を解説します。
...
本章は、OCI上でHPCワークロードを実行する際の有的な情報を提供するウェブサイトへのリンク集を今後提供していく予定です。  
情報提供開始まで、暫くお待ちください。
...
クラスタ・ネットワークの作成は、 **[インスタンス構成](#5-7-インスタンス構成)** とインスタンス数を指定して行い、このインスタンス構成に紐づく **[インスタンス・プール](#5-8-インスタンスプール)** が自動的に作成され、このインスタンス・プールがクラスタ・ネットワークに接続するインスタンスをデプロイします。  
...
マーケットプレイスは、 **[スタック](#5-3-スタック)** と **[カスタム・イメージ](#5-6-カスタムイメージ)** を提供する、オラクルが運営するオンライン・ストアです。
...
クラスタオートスケーリングは、 **[マーケットプレース](#5-5-マーケットプレイス)** から提供されるクラスタ構築用 **[スタック](#5-3-スタック)** に含まれるPythonとシェルで書かれたスクリプト群で実現され、このスタックの **Autoscaling** フィールドの **Scheduler based autoscaling** チェックボックスをオンにしてスタックを適用すると、これらのスクリプト群がインストールされて利用可能になります。

クラスタオートスケーリングを含むクラスタ構築用スタックは、 **[ここ](https://cloud.oracle.com/marketplace/application/67628143/)** を参照ください。