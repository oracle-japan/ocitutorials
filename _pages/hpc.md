...
|                 | 利点                                                          | 欠点                                                       | 備考                            |     |
| --------------- | ----------------------------------------------------------- | -------------------------------------------------------- | ----------------------------- | --- |
| 自動構築<br>（スタック）      | ユーザの作業時間(\*1)が短い<br>GUIによる操作(\*2)が可能                              | 構築手順のブラックボックス化<br>　・システム構成の変更が難しい<br>　・問題発生時原因究明難    | \*1) スタックメニュー選択の時間<br>\*2) OCIコンソール操作 |     |
| 自動構築<br>（Terraform） | ユーザの作業時間(\*3)が短い<br>CLIによる操作(\*4)が可能                                  | Terraform実行環境(\*5)が必要<br>Terraformスクリプト構文習得要(\*6) |\*3) Terraformスクリプト内変数修正<br>に要する時間<br>\*4) Terraform CLIとLinuxコマンド群<br>\*5) インターネットに接続された<br>Linux・Windows・Mac<br>\*6) ひな型Terraformスクリプトから<br>大きく逸脱する構成変更の場合                               |     |
| 手動構築<br>（OCIコンソール）  | 構築手順が明確<br>　・システム構成の変更が容易<br>　・問題発生時原因究明容易<br>GUIによる操作(\*7)が可能 | ユーザの作業時間が長い                                              |\*7) OCIコンソール操作                               |     |
