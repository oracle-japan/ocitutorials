...
DWROLEを付与する事にとって、後ほど行うデータ・ロードなどを行うことができる様になります。
...
14. レクサーの設定を行います。レクサーは文字列をトークンという細かい単位に分割してくれるものです。Oracle Textを使って全文検索を行う上で重要になります。
...
15．次に、テキスト索引を作成します。データストア型を**DIRECT_DATASTORE**とし、各列が単一のドキュメントとして索引付けします。更新頻度は**COMMIT毎**と設定します。
