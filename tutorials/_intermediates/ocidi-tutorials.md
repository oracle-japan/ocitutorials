...
## VCNとサブネットの作成
Data Integrationで利用するコンパートメントでVCNとサブネット(リージョナル）を作成します。手順は["その2-クラウドに仮想ネットワーク(VCN)を作る"](../../beginners/creating-vcn/){:target="_blank"}をご確認ください。作成したサブネットから利用するAutonomous Database、オブジェクト・ストレージへ接続可能である必要があるため、サービス・ゲートウェイをターゲット・タイプに含めるルート・ルールをルート表に追加します。
...
> 本文書ではポリシー・ビルダーを使用しますが、実際は必要なセキュリティにあわせて設定を行ってください。
...
1. 条件を以下のように修正し、作成をクリックします。  
    ...
    > 現行リリースでは条件や式にマルチバイト文字が含まれていると保存できないという問題があるため、属性名を\`で囲む必要があります。上の場合は、フィルタ_1.CUSTOMERS_JSON.COUNTRY_CODEを\`で囲んでください。
...
>保存時に次のエラーが出力された場合は条件ビルダーや式ビルダ―でマルチバイトの属性名が`で囲まれているかどうかを確認してください    
>
>![ＮULLエラー](null.png)
