...
この章では、OCI GoldenGateの作成とDBCSからADBへのデータ連携の設定について紹介します。
...
**前提条件 :**
本チュートリアルではDBCS、ADBともにデータベースの作成が完了しており、初期データとしてHRスキーマがそれぞれのデータベースにロードされていることを前提にしています。
各データベースの作成方法やデータロードの方法は下記手順をご確認ください。

 + DBCSの作成については、[「101: Oracle Cloud で Oracle Database を使おう(DBCS)」](https://oracle-japan.github.io/ocitutorials/database/dbcs101-create-db/) をご参照ください。
 + データ連携用のサンプルデータはHRスキーマを使用しています。DBCSでのHRスキーマ作成方法は、[「301: 移行元となるデータベースを作成しよう」](https://oracle-japan.github.io/ocitutorials/database/adb301-create-source-db/) をご参照ください。
 ...
 + チュートリアルの便宜上Autonomous Databaseへの接続文字列は「atp01_low」、DBCSを含めて各ユーザのパスワードはすべて「Welcome#1Welcome#1」とします。
...
# 1. ソース・データベースの設定
ソース・データベースであるDBCSに対して下記の設定を行います。
...
1. データベースの登録 詳細入力欄に、各項目を入力します。まずはソースDBとなるDBCSの登録を行っていきます。
    - **名前** - 任意
    - **別名** - 任意
    - **説明** - 任意
    - **コンパートメント** - 任意
    ...
    <br>
    今回はOCI上のDBCSをソースDBとして登録するため、**データベースの選択** を選択します。
    - **データベース・タイプ** - OCI上のどのデータベースを登録するかを選択します。<br>
    今回はOCI上のDBCSをソースDBとして登録するため、**DBシステム・データベース(ベア・メタル、VM、Exadata)** を選択します。

    - **データベース・システム** - 登録したいデータベース・システムを選択します。
    DBCSの場合データベース・システムを選択することでデータベース接続文字列まで自動入力されます。
    - **データベース・ホーム** - 任意
    - **データベース** - 任意
    - **データベースFQDN** - 任意
...
# 5. Extractの作成
ここからはOCI GGのコンソール画面に接続し、DBCSからデータを抽出するためのExtractを作成していきます。
...
1. 追加したいデータベースの **データベースへの接続** を選択します。今回はソースデータベースであるDBCSへ接続します。
...
1. ソース・データベースDBCSのemployees表を1件更新します。  
