...
- [8. ユーザ・ロールのためのAECsを設定](#8-ユーザ・ロールのためのaecsを設定)
    - [8-1. ユーザへのAECs設定](#8-1-ユーザへのaecs設定)
    - [8-2. ロールへのAECs設定](#8-2-ロールへのaecs設定)
...
“説明”に”DBMD_CLOUD用 Token”と入力し、”トークンの生成”をクリックします。
...
- [7-1. ユーザへの権限付与](#7-1-ユーザへの権限付与)を実行した場合、config_aces_for_user.sqlを編集・実行し、ACEsを設定します　→　[8-1. ユーザへのAECs設定](#8-1-ユーザへのaecs設定)へ

- [7-2. ロールへの権限付与](#7-2-ロールへの権限付与)を実行した場合、config_aces_for_role.sqlを編集・実行し、ACEsを設定します。→　[8-2. ロールへのAECs設定](#8-2-ロールへのaecs設定)へ


## 8-1. ユーザへのAECs設定

[7-1. ユーザへの権限付与](#7-1-ユーザへの権限付与)を実行した場合、以下の操作を行いAECsを設定します。

対象ユーザにAECsの設定をするために、config_aces_for_user.sqlを編集します。
<br>
...
[7-2. ロールへの権限付与](#7-2-ロールへの権限付与)を実行した場合、以下の操作を行いAECsを設定します。

対象ロールにAECsの設定をするために、config_aces_for_role.sqlを編集します。
<br>
