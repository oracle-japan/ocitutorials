...
**前提条件 :** オンデマンドGPUクラスタを収容するコンパートメント(ルート・コンパートメントでもOKです)の作成と、このコンパートメントに対する必要なリソース管理権限がユーザーに付与されていること。具体的には、以下ページの **Policies to deploy the stack:** に記載のポリシーが作成されていること。
...
また **クラスタオートスケーリング** は、Bastionノードから **Terraform** を使用して動的にGPUクラスタを構築するため、OCIの **APIキー** を作成してこれをOCIに登録する必要があります。
