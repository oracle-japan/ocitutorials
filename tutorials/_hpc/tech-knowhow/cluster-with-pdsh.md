...
これらのオペレーションは、全計算/GPUノードでsudoで管理者権限を利用可能なユーザ（ **Oracle Linux** では **opc** ）からパスフレーズ無しでSSH接続できるように構築されたクラスタ管理ノードを用意し、このノードから管理対象ノードの計算/GPUノードにSSHでコマンドを発行してこれを実施するのが一般的です。  
...
**pdsh** は、クラスタ管理ノードから管理対象ノードに対してSSHでコマンドを発行するため、パスフレーズ無しでSSHコマンドを実行できるよう事前にセットアップする必要があります。また管理オペレーションは、通常管理者権限を必要とするため、このパスフレーズ無しで実行するSSHコマンドは、管理者権限を持ったユーザから実行できるようにするか、sudoコマンドでパスワードを入力することなく管理者に昇格できるユーザである必要があります。  
...
このようにrpdcpは、コピーするファイル名に管理対象ノードのホスト名を付与したファイル名で、クラスタ管理ノードに全管理対象ノードのファイルをコピーします。
