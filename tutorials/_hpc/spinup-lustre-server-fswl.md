...
本章は、 **File Storage with Lustre** の環境を構築するために事前に作成する必要のある、以下のリソースを作成します。
...
| プライベート<br>（※5） | イングレス | いいえ    | 10.0.0.0/16 | 全てのプロトコル | -         | -       |
|        | イグレス  | いいえ    | 0.0.0.0/0   | 全てのプロトコル | -         | -       |
...
次に、ステータスが **ACTIVE** となった以下 **Lustreファイル・システム詳細** 画面で、 **Lustreプロパティ** フィールドの **マウント・コマンド** の **コピー** コマンドをクリックし、Lustreクライアントからマウントする際のコマンドを保存します。
