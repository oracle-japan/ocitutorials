...
Select AIで大規模言語モデル(LLM)を使用することで、ユーザーが入力したテキスト（自然言語）をSQLクエリに変換し、データベース内のデータを問合せることが出来ます。ユーザーは、**自然言語**を使ってデータベース内のデータと**会話**をすることが出来ます。

<br>
**目次**

- [1. OCI Generative AIのAPIキー取得](#anchor1)
- [2. 環境設定](#anchor2)
...
- [101: ADBインスタンスを作成してみよう]()を参考に、ADBインスタンスが作成済みであること
- [104: クレデンシャル・ウォレットを利用して接続してみよう]()を参考に、SQL Developperを使ってADBに接続出来ること
- OCI Generative AIを使用可能な、Chicagoリージョン、Frankfurtリージョン、Londonリージョン、Sao Pauloリージョンのいずれかをホーム・リージョン、若しくはサブスクライブしてあること。詳しくは、[Pretrained Foundational Models in Generative AI](https://docs.oracle.com/en-us/iaas/Content/generative-ai/pretrained-models.htm#)をご確認ください。
...
# 1. OCI Generative AIのAPIキー取得
Select AIでは、2024年9月現在、OpenAI、Cohere、Azure OpenAI Service、そしてOCI Genarative AIをAIプロバイダーとして使用することが出来ます。

本記事では、OCI Genarative AIをAIプロバイダーとして使用したいと思います。
...
<a id="anchor2"></a>
# 2. 環境設定
Select AIはADBの機能なので、使用にはADBインスタンスが必要です。ADBのインスタンスが未作成の場合は[101: ADBインスタンスを作成してみよう]()を参考に、ADBインスタンスが作成します。

## 2-1. ユーザーの作成と権限の付与
先ずはチュートリアルを行うユーザーの作成と、権限の付与を行います。

1. [101: ADBインスタンスを作成してみよう]()の[3-3. データベース・ユーザーを作成してみよう（コマンドライン)]()を参考に、Database ActionsのSQLワークシートから、チュートリアルを行うユーザーを作成し、必要なロールを付与します。
...
6. アップロードしたファイルを編集します。ペンのアイコンをクリックし、編集画面を開きます。
![image.png](10.png)
...
9. データセットがロードされ、HIGHSCHOOLS表が作成されました。
![image.png](13.png)

これでデータ・セットの準備が出来ました。

## 2-3. SQL Developperの設定とビューの作成

先程まで使用していたDatabase Actions内のSQLやAPEXではSELECT AIをサポートしておらず、[DBMS_CLOUD_AIパッケージのGENERATEファンクション]()を使用する必要があります。
そのため本チュートリアルでは、SQL Developperを使用して、データベースに接続したいと思います。

1. [104: クレデンシャル・ウォレットを利用して接続してみよう]()を参考に、作成したADBにSQL Developperから接続します。

2. 接続が完了したら、SQL Developper上で以下のクエリを実行し、HIGHSCHOOLS表とその中身を確認します。
...
- **private_key**：先程取得した秘密キーの中身をコピーペースト
...
/
```
これでクレデンシャルを作成する事が出来ました。このクレデンシャルはこの後プロファイルは作成する際に繰り返し使用するので、 credential_nameは忘れないようにします。
...
- **provider**：oci（本チュートリアルではOCI生成AIをAIプロバイダーとして使用）
...
3.　HIGHSCHOOLS表内に何校分データが格納されているのか確認してみます。
```sql
select count(*) from highschools;
```
HIGHSCHOOLS表内には427校分データが格納されているので、正しい結果が返ってきたことが分かります。
...
5.　showsqlオプションを付けて、どの様なSQLクエリが生成されているのか確認します。
```sql
select ai showsql マンハッタン市内に高校は全部で何校ありますか;
```
![alt text](17.png)

以下のようなSQLクエリが返ってきました。
...
表、列、ビューにコメントを追加することで、LLMに追加の情報を与えることが出来ます。BOROUGH列にコメントを追加し、カタカナで'マンハッタン'と指定してくもらえるように調整を行います。

## 3-2. コメント有りの場合
先程作成したビューの列にコメントを追加し、LLMが日本語でWHERE句を指定してくれるように補足します。
...
プロファイル名をGENAI_COHERE_COMMAND_R_COMMENTSとし（同じ名前のプロファイルは作成出来ないので注意して下さい）、"comments":"true"を追加します。
...
結果が返ってきました。生成されたSQLクエリも確認してみます。
...
以下のようなSQLクエリが返ってきました。きちんとカタカナで'マンハッタン'と指定してくれています。
```sql
...
コメントを付けて補足すると、結果の精度が向上したことが分かりました。
...
SQLクエリを実行した時の結果がAIプロバイダに送信され、自然言語でのサマリーが生成されます。
...
今まではデータベース上のデータをもとにした回答が返って来ましたが、今回はインターネット上の情報をもとにした回答が返ってきました。
