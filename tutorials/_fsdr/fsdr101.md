...
- **ディザスタ・リカバリ保護グループ(DR保護グループ)** 
  ...
  DR保護グループは、プライマリ・リージョン(本番環境)とスタンバイ・リージョン(災対環境)にそれぞれ1つずつ作成し、それらは関連付けされピア関係を形成します。
- **ディザスタ・リカバリ計画(DR計画)** 
  ...
  - フェイルオーバー(計画外)
    ...
    フェイルオーバー計画は、プライマリ・リージョンでのサービス停止をせず、スタンバイ・リージョンでアプリケーション・スタックを起動することで即時遷移を実行します。
  ...
  - ドリルの開始
    ...
    DRドリルは、災対環境が正常に起動できるかどうかをチェックするために実行されます。
...
- 移動インスタンス
    ...
    ユーザーはプライマリ・リージョンにのみインスタンスをデプロイします。

- 非移動インスタンス

    非移動インスタンスは、DR操作中に、DR保護グループからそのピアDR保護グループに移動されません。
    DR計画の実行時に、プライマリ・リージョンとスタンバイ・リージョンのインスタンスを起動もしくは停止することで、サービスの切り替えを実現します。
    インスタンスは、ユーザーによって事前にプライマリ・リージョンとスタンバイ・リージョンの両方でデプロイされる必要があります。
