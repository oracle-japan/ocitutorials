var store = [{
        ...
        "excerpt":"本ドキュメントは、HPCワークロードの実行に最適な、高帯域・低遅延RDMA対応RoCEv2採用の クラスタ・ネットワーク でHPCワークロード向けベアメタルインスタンスをノード間接続するHPCクラスタで、標準ベンチマークの Intel MPI Benchmark を実行する方法を解説します。 0. 概要 本ドキュメントで解説する Intel MPI Benchmark の実行は、HPC クラスタネットワーキングイメージ に含まれる OpenMPI と Intel MPI Benchmark を使用する方法と、 Intel oneAPI HPC Toolkit （本ドキュメントで使用するバージョンは2023.2）をインストールしてこれに含まれる Intel MPI Library （本ドキュメントで使用するバージョンは2021.10）と Intel MPI Benchmark を使用する方法を、それぞれ解説します。 また Intel MPI Benchmark は、2ノード間のPing-Pongと4ノード間のAll-Reduceの計測方法をそれぞれ解説します。 本ドキュメントで Intel MPI Benchmark を実行するHPCクラスタは、HPCワークロード向けベアメタルシェイプ BM.Optimized3.36 4インスタンスを クラスタ・ネットワーク で接続した構成とし、 OCI...","categories": [],
        "tags": [],
