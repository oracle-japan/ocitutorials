var store = [{
        ...
        "excerpt":"0. 概要 本パフォーマンス関連Tipsは、 BM.Optimized3.36 を クラスタ・ネットワーク でノード間接続するHPCクラスタで OpenFOAM を使用する際、特定のモデルを使用するCFD解析フローをコストパフォーマンス良く実行するという観点で、以下のTipsを解説します。 ノード内並列とマルチノード並列に於けるMPIプロセス配置を最適化する方法 NVMe SSDローカルディスクをストレージ領域に活用する方法 本パフォーマンス関連Tipsの性能計測は、 OCI HPCテクニカルTips集 の OpenFOAMインストール・利用方法 に従って構築された OpenFOAM 実行環境を使用し、 OCI HPCテクニカルTips集 の Slurmによるリソース管理・ジョブ管理システム構築方法 に従って構築された Slurm 環境でバッチジョブとして計測しています。 また、計算ノードに使用する BM.Optimized3.36 は、 OpenFOAM がメモリ帯域幅依存でハイパースレッディングによる効果は期待できないため、 OCI HPCパフォーマンス関連情報 の パフォーマンスに関連するベアメタルインスタンスのBIOS設定方法 の手順に従い SMT を無効化しています。 1. ノード内並列とマルチノード並列に於けるMPIプロセス配置を最適化する方法 1-0. 概要 本Tipsは、 OpenFOAM のの実行性能がメモリ帯域幅に大きく影響されることを念頭に、メモリ性能に影響する以下のパラメータを調整することでどのように性能が変化するか、また最も性能の良いパラメータは何かという観点で性能を計測します。 NPS （Numa node Per...","categories": [],
        "tags": [],
