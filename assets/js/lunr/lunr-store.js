var store = [{
        ...
        "title": "HPL実行方法",
        "excerpt":"本ドキュメントは、HPCワークロードの実行に最適な、高帯域・低遅延RDMA対応RoCEv2採用の クラスタ・ネットワーク でHPCワークロード向けベアメタルインスタンスをノード間接続するHPCクラスタで、標準ベンチマークの HPL を実行する方法を解説します。 0. 概要 本ドキュメントで解説する HPL の実行は、 Intel oneAPI Math Kernel Library for Linux （本ドキュメントで使用するバージョンは2023.2）に含まれるIntelの HPL 実装である Intel Distribution for LINPACK Benchmark を Intel MPI Library （本ドキュメントで使用するバージョンは2021.10）と共に使用します。 なお、 Intel oneAPI Math Kernel Library for Linux と Intel MPI Library は、 Intel oneAPI HPC Toolkit （本ドキュメントで使用するバージョンは2023.2）に含まれているものを使用します。 本ドキュメントで HPL...","categories": [],
        "tags": [],
        "url": "/ocitutorials/hpc/benchmark/run-hpl/",
        "teaser": null
      },{
        "title": "STREAM実行方法",
        "excerpt":"本ドキュメントは、HPCワークロードの実行に最適なベアメタルインスタンスで、標準ベンチマークの STREAM を実行する方法を解説します。 0. 概要 本ドキュメントで解説する STREAM の実行は、 Intel oneAPI Base Toolkit （本ドキュメントで使用するバージョンは2023.2）に含まれるCコンパイラの Intel oneAPI DPC++/C++ Compiler （本ドキュメントで使用するバージョンは2023.2）で STREAM のソースコードをコンパイルして作成したバイナリを使用し、HPCワークロード向けベアメタルシェイプ BM.Optimized3.36 を Oracle Linux 8ベースの HPCクラスタネットワーキングイメージ でデプロイし、この上で実行します。 この際、 STREAM の性能向上を目的とし、インスタンスをデプロイする際のBIOS設定で NUMA setting を NPS2 とします。 以上より、本ドキュメントで解説する STREAM 実行は、以下の手順を経て行います。 BM.Optimized3.36 インスタンスのデプロイ Intel oneAPI Base Toolkit インストール STREAM ダウンロード・コンパイル STREAM 実行 本ドキュメントは、以下の環境で...","categories": [],
        "tags": [],
