var store = [{
        ...
        "excerpt":"はじめに このチュートリアルでは、ファイルからテキストへ、テキストからチャンクへ、チャンクからベクトルデータへの変換、およびベクトルの検索について、ステップバイステップでの実行する方法をご紹介します。 23ai free、ADB23ai(Always Free)の両方でのやり方をご紹介します。 ファイルからテキストへ変換する関数セット(UTL_TO_TEXTおよびDBMS_VECTOR_CHAIN)を使用して、テキストをチャンク(UTL_TO_CHUNKS)に分割し、次にベクトル化モデルを使ってそれぞれのチャンクのベクトル(UTL_TO_EMBEDDINGS)を作成します。 0. 前提条件 ベクトル化処理ではOCI Generative AIのサービスを使用しますので、下記の前提条件を満たせる必要があります。 使用するOracle Databaseの環境があること 23ai freeで実行する場合、Oracle Database 23ai Freeをインストールする済みであること ※インストール方法については、102 : 仮想マシンへOracle Database 23ai Freeをインストールしてみよう を参照ください。 ADB23aiで実行する場合、Autonomous Database 23aiのインスタンスを構築済みであること。本記事では無償で使えるAlways Freeを使います。 ※インスタンス作成方法については。101 : Always Freeで23aiのADBインスタンスを作成してみようをご参照ください。 OCI GenAIのサービスをご利用いただけるChicagoのRegionはサブスクリプション済みであること。 目次： はじめに 0. 前提条件 1. 事前準備 Oracle Database 23ai free編 1-1. Oracle Database 23ai Free編-ファイルの格納...","categories": [],
        "tags": [],
