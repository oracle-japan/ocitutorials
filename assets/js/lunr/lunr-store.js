var store = [{
        ...
        "excerpt":"0. 概要 HPC/GPUクラスタと共に利用するファイル共有ストレージは、その構築方法を決定する際に以下の評価項目を考慮する必要があります。 計算/GPUノードの複数ノード同時アクセス時性能 ランニングコスト ストレージ領域に格納するデータの可用性 ファイル共有ストレージサービスの可用性 システム構築・運用のしやすさ 本テクニカルTipsは、HPC/GPUクラスタ向けにNFSでサービスするファイル共有ストレージを構築することを念頭に、いくつかのファイル共有ストレージ構築手法を紹介し、上記評価基準を元にどの手法を採用すればよいか、その考慮点を解説します。 NFSでサービスするファイル共有ストレージを構築する際の最初の考慮点は、マネージドのNFSサービスである ファイル・ストレージ を使用する方法と、自身で ベア・メタル・インスタンス からNFSサーバを構築する（以降”ベア・メタル・インスタンスNFSサーバ”と呼称）方法の、どちらを採用するかです。 ここで ファイル・ストレージ を採用する場合は、性能要件に合わせて マウント・ターゲット に標準タイプ（以降”標準FSS”と呼称）とHigh Performanceタイプ（以降”高性能FSS”と呼称）（※1）のどちらを使用するかを考慮します。 ※1）High Performanceタイプの マウント・ターゲット は、その性能違いで3種類存在しますが、本テクニカルTipsでは読み取りスループット設定が80 Gbpsで性能が最も高い HPMT-80 を前提に解説します。 High Performanceタイプの マウント・ターゲット 詳細は、 ここ を参照くして下さい。 またベア・メタル・インスタンスNFSサーバを採用する場合は、以下2種類のどちらにするかを考慮します。 ストレージ領域に ブロック・ボリューム を使用しこれをベア・メタル・インスタンスにアタッチする方法（以降”ブロック・ボリュームNFSサーバ”と呼称） ベア・メタル・インスタンス にNVMe SSDドライブを搭載するDenceIOシェイプを使用する方法（以降”DenceIO NFSサーバ”と呼称） 以上より、本テクニカルTipsで解説するファイル共有ストレージは、以下4種類です。 呼称 使用するサービス ヘッドノード ストレージ領域 ターゲットスループット 標準FSS ファイル・ストレージ 標準マウント・ターゲット...","categories": [],
        "tags": [],
