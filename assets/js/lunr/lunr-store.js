var store = [{
        ...
        "excerpt":"Oracle Cloud Infrastructure（以降OCIと記載）は、仮想化オーバーヘッドの無いHPC用途に特化したベアメタルシェイプと、これらを高速・低遅延で接続するインターコネクトネットワークサービスの クラスタ・ネットワーク を提供しており、HPCワークロードを実行するHPCクラスタを構築するには最適なクラウドサービスです。 このチュートリアルは、 マーケットプレイス から無償で利用可能な HPCクラスタスタック を利用し、以下構成の典型的なHPCクラスタを構築します。 計算ノード： HPCワークロード向けIntel Ice Lakeプロセッサ搭載ベアメタルシェイプの BM.Optimized3.36 インターコネクトネットワーク: クラスタ・ネットワーク （ノード当たり100 Gbps x 1） インターネットからSSH接続可能なBastionノード OS: Oracle Linux 8 ジョブスケジューラ: Slurm クラスタ内ホームディレクトリ共有： ファイル・ストレージ クラスタ内ユーザ統合管理： LDAP またこのチュートリアルは、デプロイしたHPCクラスタのインターコネクト性能を Intel MPI Benchmark で確認します。 またこのチュートリアルでは、クラスタ構築後により大規模な計算を実施する必要が生じたり、メンテナンスによりノードを入れ替える必要が生じることを想定し、既存のクラスタに計算ノードを追加する方法と、特定の計算ノードを入れ替える方法も学習します。 本チュートリアルで使用する HPCクラスタスタック は、通常であれば数日かかるHPCクラスタ構築作業を、OCIコンソールのGUIから10項目程度のメニューを選択するだけで実施することを可能にします。 所要時間 : 約1時間 前提条件 : HPCクラスタを収容する コンパートメント ( ルート・コンパートメント...","categories": [],
        "tags": [],
        ...
        "excerpt":"Oracle Cloud Infrastructure（以降OCIと記載）は、仮想化オーバーヘッドの無いHPC用途に特化したベアメタルシェイプと、これらを高速・低遅延で接続するインターコネクトネットワークサービスの クラスタ・ネットワーク を提供しており、HPCワークロードを実行するHPCクラスタを構築するには最適なクラウドサービスです。 このチュートリアルは、 マーケットプレイス から無償で利用可能な HPCクラスタスタック を利用し、以下構成のオンデマンドHPCクラスタを構築します。 計算ノード： HPCワークロード向けIntel Ice Lakeプロセッサ搭載ベアメタルシェイプの BM.Optimized3.36 インターコネクトネットワーク: クラスタ・ネットワーク （ノード当たり100 Gbps x 1） インターネットからSSH接続可能なBastionノード OS: Oracle Linux 8 ジョブスケジューラ: Slurm オンデマンドクラスタ機能： クラスタオートスケーリング クラスタ内ホームディレクトリ共有： ファイル・ストレージ クラスタ内ユーザ統合管理： LDAP またこのチュートリアルは、デプロイしたHPCクラスタのインターコネクト性能を Intel MPI Benchmark で確認します。 オンデマンドHPCクラスタにおけるワークロード実行は、 Slurm にジョブを投入することで行い、 クラスタオートスケーリング がジョブ実行に必要な計算ノードを クラスタ・ネットワーク と共に動的にデプロイし、構築されたクラスタに Slurm がジョブをディスパッチします。 また クラスタオートスケーリング...","categories": [],
        "tags": [],
        ...
        "excerpt":"本ドキュメントは、HPCワークロードの実行に最適な、高帯域・低遅延RDMA対応RoCEv2採用の クラスタ・ネットワーク でHPCワークロード向けベアメタルインスタンスをノード間接続するHPCクラスタで、標準ベンチマークの Intel MPI Benchmark を実行する方法を解説します。 0. 概要 本ドキュメントで解説する Intel MPI Benchmark の実行は、 クラスタネットワーキングイメージ に含まれる OpenMPI （本ドキュメントで使用するバージョンは4.1.2a1）と Intel MPI Benchmark を使用する方法と、 Intel oneAPI HPC Toolkit （本ドキュメントで使用するバージョンは2023.2）をインストールしてこれに含まれる Intel MPI Library （本ドキュメントで使用するバージョンは2021.10）と Intel MPI Benchmark を使用する方法を、それぞれ解説します。 また Intel MPI Benchmark は、2ノード間の Ping-Pong と4ノード間の All-Reduce の計測方法をそれぞれ解説します。 本ドキュメントで Intel MPI Benchmark を実行するHPCクラスタは、HPCワークロード向けベアメタルシェイプ BM.Optimized3.36...","categories": [],
        "tags": [],
        ...
        "excerpt":"クラスタ・ネットワーク を使用するインスタンスは、接続に必要なソフトウェアがインストールされている必要がありますが、これらを含んだOSイメージが クラスタネットワーキングイメージ として マーケットプレイス から提供されています。 本テクニカルTipsは、この クラスタネットワーキングイメージ の適切な選び方を解説します。 0. 概要 クラスタネットワーキングイメージ は、ベースOSにOracle Linuxを採用し、シェイプがBM.HPC2.36/ BM.Optimized3.36 の場合そのバージョンは7.9と8から選択でき、 BM.GPU4.8/BM.GPU.A100-v2.8 の場合7.9のみ選択可能です。 なお、2023年6月29日時点のバージョン8系 クラスタネットワーキングイメージ の最新は、8.7です。 下表は、クラスタ・ネットワークに対応したシェイプと対応する クラスタネットワーキングイメージ の組み合わせを示しています。 シェイプ OSイメージ名 ベースOS OFED マーケットプレイス URL BM.HPC2.36 HPC Cluster Networking Image Oracle Linux 7.9 5.4 Link （※1）   HPC Cluster Networking Image Oracle Linux 8 5.4...","categories": [],
        "tags": [],
        "url": "/ocitutorials/hpc/tech-knowhow/osimage-for-cluster/",
        "teaser": null
      },{
        "title": "クラスタ・ネットワーク非対応OSイメージを使ったクラスタ・ネットワーク接続方法",
        "excerpt":"クラスタ・ネットワーク に接続するインスタンスは、接続に必要なソフトウェアがインストールされている必要があり、これらを含んだOSイメージが クラスタネットワーキングイメージ として マーケットプレース から提供されていますが、これらのベースとなるOSは、Oracle Linuxのみです。 本テクニカルTipsは、 クラスタ・ネットワーク 対応シェイプの BM.Optimized3.36 や BM.GPU4.8/BM.GPU.A100-v2.8 を使用するインスタンスを Oracle Linux 以外のOSで クラスタ・ネットワーク に接続する方法を解説します。 注意 : 本コンテンツ内の画面ショットは、現在のOCIコンソール画面と異なっている場合があります。 0. 概要 クラスタ・ネットワーク に接続するインスタンスは、以下の条件を満たす必要があります。 クラスタ・ネットワーク 対応シェイプ（※1）を使用している クラスタ・ネットワーク のデプロイに伴ってデプロイされている クラスタ・ネットワーク 接続のための以下ソフトウェアがインストールされている Mellanox OFED wpa_supplicant（※2） oci-cn-auth（※3） クラスタ・ネットワーク 接続用ネットワークインターフェースがOS上で構築されている ※1）クラスタ・ネットワーク 対応シェイプは、以下に記載があります。 https://docs.public.oneportal.content.oci.oraclecloud.com/ja-jp/iaas/Content/Compute/Tasks/managingclusternetworks.htm#supported-shapes ※2）クラスタ・ネットワーク は、インスタンスが接続する際802.1X認証を要求しますが、この処理を行うクライアントソフトウェアがwpa_supplicantです。802.1X認証の仕組みは、以下のサイトが参考になります。 https://www.infraexpert.com/study/wireless14.html ※3）クラスタ・ネットワーク に接続する際の802.1X認証で必要な認証処理機能を提供するユーティリティーソフトウェアで、GitHubから公開されています。 これらの条件の中で、 クラスタ・ネットワーク 接続のためのソフトウェアは、ベースOSに Oracle...","categories": [],
        "tags": [],
        ...
        "excerpt":"ノード数が多くなるHPCクラスタやGPUクラスタの計算/GPUノードの名前解決は、どのように行うのが効果的でしょうか。 本テクニカルTipsは、 仮想クラウド・ネットワーク のDNSを使用した効果的な計算/GPUノードの名前解決方法を解説します。 注意 : 本コンテンツ内の画面ショットは、現在のOCIコンソール画面と異なっている場合があります。 0. 概要 仮想クラウド・ネットワーク に接続するインスタンスは、 仮想クラウド・ネットワーク が提供するDNSサービスにインスタンス名とIPアドレスの正引き・逆引き情報がデプロイ時に自動的に登録され、インスタンス名を使用して名前解決することが可能です。 このため、ホスト名とIPアドレスの関係をシステム管理者がメンテナンスする/etc/hostsやNISと比較し、DNS名前解決を活用することでホスト名管理を省力化することが可能です。 ここで、 仮想クラウド・ネットワーク が提供するDNSサービスは、インスタンス名（compute1）、接続する 仮想クラウド・ネットワーク 名（vcn）、及びサブネット名（private）を使用し、インスタンスのFQDNを以下のように登録します。 compute1.private.vcn.oraclevcn.com このため、DNS名前解決対象のインスタンスが接続するサブネットのFQDNを/etc/resolv.confのsearch行に追加することで、インスタンス名によるDNS名前解決が可能になります。 例えば、 仮想クラウド・ネットワーク 名がvcn、サブネット名がpublicのサブネットに接続されるBastionノード上で、同じ 仮想クラウド・ネットワーク のサブネット名がprivateのサブネットに接続される計算ノードをインスタンス名でDNS名前解決する場合、Bastionノードの/etc/resolv.confのsearch行が以下となっていれば良いことになります。 search vcn.oraclevcn.com public.vcn.oraclevcn.com private.vcn.oraclevcn.com ただ、この方針に沿って/etc/resolv.confを修正する際、このファイルがDHCPクライアントにより管理されており、DHCPのリース切れやOS再起動の際に修正が元に戻ってしまうことに注意する必要があります。 以上より、インスタンス名によるDNS名前解決は、以下の手順を経て行います。 サブネットFQDN確認: DNS名前解決対象のインスタンスが接続するサブネットのFQDNの確認 resolv.confファイル修正: DNS名前解を行うインスタンスのresolv.confの修正 1.サブネットFQDN確認 本章は、インスタンス名でDNS名前解決を行いたいインスタンスが接続されるサブネットのFQDNを確認します。 サブネットのFQDNは、OCIコンソールでサブネットが存在するリージョンを選択後、 ネットワーキング → 仮想クラウド・ネットワーク とメニューを辿り、表示される以下画面で該当するサブネットが含まれる 仮想クラウド・ネットワーク を選択します。 表示される以下画面で、該当するサブネットを選択します。 表示される以下 サブネット詳細 画面の DNSドメイン名...","categories": [],
        "tags": [],
        ...
        "excerpt":"本資料の目的 : この文書は、お客様がトラブル解決のために技術サポート（Support RequestまたはService Request、以下SRと略します）を効果的に活用するためのポイント、記載すべき項目、伝える際の注意事項をまとめたものです。サポート側は、個々の問い合わせに誠実に取り組み、解決に向けて最善の努力を尽くします。サポート側とお客様が協力し合うことは、トラブルの早期解決に寄与します。これらはすべて任意の内容ですので、必ずしも全てを実施しなければならないわけではありません。なにをどこまで参考にするかはそれぞれのご判断で取り入れていただけますようお願いします 前提条件 : OCIを利用するお客様・パートナー様のエンジニア向け。ただし技術サポートツールの「操作方法」については本文書には含みませんので、下記各種ガイドをご参照ください (製品別サポート窓口／ご利用ガイド) (My Oracle Support サービス・リクエスト作成ガイド) (Oracle Cloud Supportご利用ガイド) 目次 1. トラブル解決に向けた全体像と技術サポート(SR)の位置付け 2. SRを用いたトラブル解決のコツ 3.1 SR起票のコツ(テンプレートの利用) 3.2 SR起票のコツ(起票時項目のチェックリスト)　 所要時間： 約10分 1. トラブル解決に向けた全体像と技術サポート(SR)の位置付け 1.1 トラブル解決とは 目の前で発生している「問題事象」を解決するためには、まず当然ながら、その「被疑箇所」を特定し、発生原因を明らかにします。その後、暫定的な回避策を適用し、同時に恒久的な対策が有効であることを確認・実施して、トラブルを解決できたと言えます 1.2 トラブル解決までの流れ 「問題事象の切り分け」や「ログ取得」などの各工程は、解決後に振り返ると一本道の活動のように見えるかもしれません。実際には、解決の過程において、ログを読み込み・分析し、さまざまな可能性を推定しながら疑義箇所を絞り込み・判断するなど、原因が特定できるまでは、追加調査や再調査が必要で、反復的な工程が含まれることがあります 1.3 関係者の役割分担と相互協力 トラブル解決のための技術サポート体制は、有料アカウントをお持ちのすべてのお客様に自動付帯される 「Oracleサポート」 が基本となります。加えて、プロアクティブにトラブル解決を行うサポートとして 「Oracleカスタマーサクセスサービス(CSS)」 が利用可能です 留意点：他クラウドとのサポート利用体系の違い 他の一般的なクラウドサービスプロバイダーでは、クラウド契約時にサポートプランを選択することでサポート利用が可能になります（月額クラウド利用額に比率を乗じたサポート利用料が発生）。それらとは異なり、OCIではお客様が契約時にサポートプランを選択することなく(サポート利用料を発生させずに)基本的な技術サポートを利用開始できる体系です 「Oracleサポート」の主な役割と役割分担の考え方 「Oracleサポート」 は、お客様が提起したSRに対して、解決への道筋として「ログ取得」をお客様に依頼します。しかし、「切り分け」や「ログ取得」の段階では、一度の作業で「原因の特定」に至ることは必ずしもありません。そのため、 お互いに協力的な調査と対応を行うための双方向のコミュニケーション が、早期解決のカギとなります 「Oracleサポート」...","categories": [],
        "tags": [],
