var store = [{
        ...
        "excerpt":"0. 概要 HPC/GPUクラスタを運用する際必須となるファイル共有ストレージは、NFSでこれをサービスすることが一般的ですが、このファイル共有ストレージをコストパフォーマンス優先で選定する場合、 ベア・メタル・インスタンス とストレージサービスで構築する方法（以降”ベア・メタル・インスタンスNFSサーバ”と呼称）を採用することになり、ストレージに ブロック・ボリューム を使用しこれをベア・メタル・インスタンスにアタッチする方法（以降”ブロック・ボリュームNFSサーバ”と呼称）と、 ベア・メタル・インスタンス にNVMe SSDドライブを搭載するDenceIOシェイプを使用する方法（以降”DenceIO NFSサーバ”と呼称）があります。（※1） ※1）ベア・メタル・インスタンスNFSサーバの詳細は、 OCI HPCテクニカルTips集 の HPC/GPUクラスタ向けファイル共有ストレージの最適な構築手法 を参照してください。 このベア・メタル・インスタンスNFSサーバは、NFSのマネージドサービスである ファイル・ストレージ の場合は備え付けのバックアップ機能を利用できるのに対し、自身でバックアップ環境を構築する必要があります。（※2） ※2）バックアップの観点でのベア・メタル・インスタンスNFSサーバと ファイル・ストレージ の比較は、 OCI HPCテクニカルTips集 の HPC/GPUクラスタ向けファイル共有ストレージの最適な構築手法 の 2-2 可用性による比較 を参照してください。 以上を踏まえて本チュートリアルは、ベア・メタル・インスタンスNFSサーバで構築するHPC/GPUクラスタのファイル共有ストレージに格納されるファイルをバックアップする、バックアップサーバの構築方法を解説します。 ここでバックアップを格納するストレージは、通常バックアップ対象のファイルを格納するストレージ（ ブロック・ボリューム やNVMe SSDドライブ）より容量単価の安価なものを選択する必要があるため、 オブジェクト・ストレージ を使用します。 この際のバックアップツールは、 オブジェクト・ストレージ とPOSIXファイルシステム間のデータ転送ツールとして幅広く利用されているオープンソースの Rclone を採用し、ベア・メタル・インスタンスNFSサーバのNFSクライアントとして構成したバックアップサーバ上でこの Rclone のファイル同期機能を使用し、NFSマウントしたファイルシステム領域を オブジェクト・ストレージ に差分バックアップします。 Rclone が オブジェクト・ストレージ...","categories": [],
        "tags": [],
        "url": "/ocitutorials/hpc/spinup-backup-server/",
        "teaser": "/ocitutorials/hpc/spinup-backup-server/architecture_diagram.png"
      },{
