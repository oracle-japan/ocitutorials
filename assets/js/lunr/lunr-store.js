var store = [{
        ...
        "title": "STREAM実行方法",
        "excerpt":"本ドキュメントは、HPCワークロードの実行に最適なベアメタルインスタンスで、標準ベンチマークの STREAM を実行する方法を解説します。 0. 概要 本ドキュメントで解説する STREAM の実行は、 Intel oneAPI Base Toolkit （本ドキュメントで使用するバージョンは2023.2）に含まれるCコンパイラの Intel oneAPI DPC++/C++ Compiler （本ドキュメントで使用するバージョンは2023.2）で STREAM のソースコードをコンパイルして作成したバイナリを使用し、HPCワークロード向けベアメタルシェイプ BM.Optimized3.36 をOracle Linux 8ベースの HPCクラスタネットワーキングイメージ でデプロイし、この上で実行します。 以上より、本ドキュメントで解説する STREAM 実行は、以下の手順を経て行います。 Intel oneAPI Base Toolkit インストール STREAM ダウンロード・コンパイル STREAM 実行 本ドキュメントは、以下の環境で STREAM を実行しており、以下の性能が出ています。 [実行環境] シェイプ: BM.Optimized3.36 OS: Oracle Linux 8.7 (HPC クラスタネットワーキングイメージ...","categories": [],
        "tags": [],
