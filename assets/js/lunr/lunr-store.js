var store = [{
        ...
        "excerpt":"0. 概要 ベア・メタル・インスタンス と ブロック・ボリューム 等のストレージサービスで構築するNFSでサービスするファイル共有ストレージは、NFSのマネージドサービスである ファイル・ストレージ の場合は備え付けのバックアップ機能を利用できるのに対し、自身でバックアップ環境を構築する必要があります。 この際バックアップを格納するストレージは、通常バックアップ対象のファイルを格納するストレージより容量単価の安価なものを選択する必要があるため、 オブジェクト・ストレージ がその有力な候補になります。 この際バックアップを行うツールは、POSIXファイルシステムと オブジェクト・ストレージ 間の差分バックアップ機能が必要になり、この機能を有する代表的なものとして以下のツールが挙げられます。 Command Line Interface（以降”OCI CLI”と呼称） の oci os object sync コマンド OCI の操作をコマンドラインインターフェースで行うための OCI CLI は、POSIXファイルシステムと オブジェクト・ストレージ を同期させるサブコマンド oci os object sync を提供しており、これを利用して差分バックアップを実現します。 OCIFS + rsync OCI が無償で提供するユーティリティの OCIFS は、 オブジェクト・ストレージ をPOSIXファイルシステムのディレクトリ階層にマウントし、POSIXのIOシステムコールを使用するアプリケーションが オブジェクト・ストレージ にアクセスすることを可能にします。 OCIFS でマウントされた オブジェクト・ストレージ...","categories": [],
        "tags": [],
