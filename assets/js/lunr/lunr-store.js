var store = [{
        ...
        "excerpt":"HPC/機械学習ワークロードをオンプレミスからOCIに移行する際、多くのケースでオンプレミスとOCIを拠点間接続する必要が生じます。 例えば、有償CAEアプリケーションのライセンスサーバがオンプレミス側に存在し、OCI側で稼働する計算ノードでこのライセンスサーバからライセンスを取得するケースです。 本テクニカルTipsは、 サイト間VPN を使用してIPSecのトンネルモードでオンプレミスとOCIを拠点間接続し、OCI側プライベートサブネットに接続する計算ノード相当のインスタンスとオンプレミス側プライベートサブネットに接続するライセンスサーバ相当のインスタンスの疎通を可能とするための手順を解説します。 注意 : 本コンテンツ内の画面ショットは、現在のOCIコンソール画面と異なっている場合があります。 0. 概要 サイト間VPN は、IPsecのトンネルモードでオンプレミスのネットワークとOCIの 仮想クラウド・ネットワーク をIP層で拠点間接続するため、以下のような利用形態をシームレスに実現することが可能です。 OCIで稼働する計算ノードでオンプレミスのライセンスサーバから有償CAEアプリケーションのライセンスを取得する オンプレミスの端末からOCIで稼働するログインノードにログインしインタラクティブ処理を行う オンプレミスの端末からOCIで稼働するログインノードにシミュレーションに必要なファイルを転送する OCIで稼働するログインノードからシミュレーションの結果ファイルをオンプレミスの端末に転送する また サイト間VPN は、サービス自体は無償のサービスでパブリックインターネットを拠点間接続回線に利用するため、OCIからオンプレミスに向けた転送データ量が少ないケースでは、他の有料専用線サービスと比較し、コスト効果の高い拠点間接続が可能です。 なお サイト間VPN を介する拠点間の通信帯域は、使用するインターネット接続回線に十分な性能がある場合、サービス自体の性能上限として250Mbps程度を期待することが出来ます。（詳細は、 ここ を参照ください。）このため、前述の利用形態 1. や 2. のケースに於いては、最もコストパフォーマンスの良い拠点間接続方法であると言えます。 ここでオンプレミス側のVPN接続機器は、どのように用意したらよいでしょうか。 これには、以下の選択肢があります。 VPNアプライアンス製品（※1）を使用する サーバ上で動作するIPSec実装（※2）を使用する ※1） サイト間VPN サービスで検証された ここ に記載のVPNアプライアンスから選択します。 ※2） サイト間VPN サービスで検証された Libreswan か strongSwan から選択します。 このため、オンプレミスで既に サイト間VPN サービスで検証されたVPNアプライアンス製品が稼働している場合は選択肢 1....","categories": [],
        "tags": [],
