var store = [{
        ...
        "excerpt":"チュートリアル一覧に戻る : Oracle Cloud Infrastructure チュートリアル このチュートリアルでは、コンソール画面から仮想クラウド・ネットワーク(Virtual Cloud Network : VCN) を1つ作成し、その構成について確認してくことで、OCIのネットワークに対する理解を深めます。 注意 : チュートリアル内の画面ショットについては Oracle Cloud Infrastructure の現在のコンソール画面と異なっている場合があります 所要時間 : 約15分 前提条件 : Oracle Cloud Infrastructure の環境(無料トライアルでも可) と、管理権限を持つユーザーアカウントがあること その1 - OCIコンソールにアクセスして基本を理解するを完了していること 目次： 1. 仮想クラウドネットワーク(VCN)とは? 2. 複数のコンポーネント含めたVCNをウィザードで一括作成する 3 作成されたリソースを確認する 4. インターネットからインスタンスへSSH接続するときのパケットの動き 参考動画：本チュートリアルの内容をベースとした定期ハンズオンWebinarの録画コンテンツです。操作の流れや解説を動画で確認したい方はご参照ください。 Oracle Cloud Infrastructure ハンズオン - 2.仮想クラウドネットワーク 1. 仮想クラウドネットワーク(VCN)とは?...","categories": [],
        "tags": [],
        "url": "/ocitutorials/beginners/creating-vcn/",
        "teaser": "/ocitutorials/beginners/creating-vcn/1_overviewVCN.png"
      },{
        "title": "その3 - インスタンスを作成する",
        "excerpt":"チュートリアル一覧に戻る : Oracle Cloud Infrastructure チュートリアル この章では、作成済みの仮想クラウド・ネットワーク（VCN）の中にコンピュート・インスタンスを作成していきます。 所要時間 : 約20分 前提条件 : その2 - クラウドに仮想ネットワーク(VCN)を作る を通じて仮想クラウド・ネットワーク(VCN)の作成が完了していること 注意 : チュートリアル内の画面ショットについては Oracle Cloud Infrastructure の現在のコンソール画面と異なっている場合があります 目次： 1. 新規仮想マシンインスタンスの作成 2. 作成したインスタンス詳細情報の確認 3. インスタンスへの接続 参考動画：本チュートリアルの内容をベースとした定期ハンズオンWebinarの録画コンテンツです。操作の流れや解説を動画で確認したい方はご参照ください。 Oracle Cloud Infrastructure ハンズオン - 3.コンピュート・インスタンス 1. 新規仮想マシンインスタンスの作成 第2章で作成した仮想コンピュート・ネットワーク（VCN）に、新しくインスタンスを作成していきます。 今回はコンソールから、一番小さなシェイプ（1 OCPU）の仮想マシン（VM）タイプの Oracle Linux 8 のインスタンスを1つ作成します。 コンソールメニューから コンピュート → インスタンス...","categories": [],
        "tags": [],
        "url": "/ocitutorials/beginners/creating-compute-instance/",
        "teaser": "/ocitutorials/beginners/creating-compute-instance/img09.png"
      },{
        "title": "その4 - ブロック・ボリュームをインスタンスにアタッチする",
        "excerpt":"チュートリアル一覧に戻る : Oracle Cloud Infrastructure チュートリアル ＜その4 - ブロック・ボリュームをインスタンスにアタッチする＞ では、コンソール画面からブロック・ボリューム と呼ばれるストレージを作成し、インスタンスにアタッチをしてストレージ領域を拡張させます。 所要時間 : 約20分 前提条件 : その2 - クラウドに仮想ネットワーク(VCN)を作る その3 - インスタンスを作成する 仮想クラウド・ネットワーク(VCN)の中に任意のLinuxインスタンスの作成が完了していること 注意 : チュートリアル内の画面ショットについては Oracle Cloud Infrastructure の現在のコンソール画面と異なっている場合があります 目次： 1. ブロック・ボリュームとは? 2. ブロック・ボリュームの作成 3 ボリュームのインスタンスへのアタッチ 4. ボリュームのフォーマットおよびマウント 参考動画： 本チュートリアルの内容をベースとした定期ハンズオンWebinarの録画コンテンツです。操作の流れや解説を動画で確認したい方はご参照ください。 Oracle Cloud Infrastructure ハンズオン - 4.ブロック・ボリューム 1. ブロック・ボリュームとは ブロック・ボリュームは、OCI上でデータを保存・格納するブロックストレージサービスです。インスタンスに「アタッチ」するとネットワーク越しにストレージを使うことができます。またインスタンスからデタッチし、新しい別のインスタンスにアタッチすることで、データを失うことなく移行することが可能です。...","categories": [],
        "tags": [],
        "url": "/ocitutorials/beginners/attaching-block-volume/",
        "teaser": "/ocitutorials/beginners/attaching-block-volume/BVoverview.png"
      },{
        ...
        "excerpt":"チュートリアル一覧に戻る : 入門編 - Oracle Cloud Infrastructure を使ってみよう ＜その7 - オブジェクト・ストレージを使う＞ では、コンソール画面からオブジェクト・ストレージと呼ばれるストレージを作成し、スタンダード・バケットの作成やオブジェクトのアップロード、ダウンロードなどの基本的な操作、また事前認証リクエストを作成して一般ユーザー向けにダウンロードリンクを生成する手順について学習します。 所要時間 : 約10分 前提条件 : 権限 : 適切なコンパートメント(ルート・コンパートメントでもOKです)と、そこに対する適切なオブジェクト・ストレージの管理権限がユーザーに付与されていること 注意 : チュートリアル内の画面ショットについては Oracle Cloud Infrastructure の現在のコンソール画面と異なっている場合があります 目次： 1. オブジェクト・ストレージとは 2. コンソール画面の確認とバケットの作成 3 オブジェクトのアップロード、ダウンロード 4. 事前認証済リクエストの作成 参考動画：本チュートリアルの内容をベースとした定期ハンズオンWebinarの録画コンテンツです。操作の流れや解説を動画で確認したい方はご参照ください。 Oracle Cloud Infrastructure ハンズオン - 7.オブジェクト・ストレージ 1. オブジェクト・ストレージとは オブジェクト・ストレージは、高い信頼性と高い費用対効果を両立するスケーラブルなクラウドストレージです。 オブジェクト・ストレージを利用すると、分析用のビッグ・データや、イメージやビデオ等のリッチ・メディア・コンテンツなど、あらゆるコンテンツ・タイプの非構造化データを無制限に保管できます。 オブジェクト・ストレージはリージョン単位のサービスで、コンピュート・インスタンスからは独立して動作します。オブジェクト・ストレージはOracle Services Network（OSN）にエンドポイントがあり、VCN内からサービスゲートウェイを経由したりインターネットを経由したりしてこのエンドポイントにアクセスすることができます。...","categories": [],
        "tags": [],
        "url": "/ocitutorials/beginners/object-storage/",
        "teaser": "/ocitutorials/beginners/object-storage/1_overview.png"
      },{
        ...
        "excerpt":"はじめに OCI Full Stack Disaster Recovery (FSDR) は、Oracle Cloud Infrastructure (OCI)のディザスタ・リカバリ・オーケストレーションおよび管理サービスであり、インフラ、ミドルウェア、データベース、アプリケーションなど、アプリケーション・スタックのすべてのレイヤーに包括的なディザスタ・リカバリ機能を提供します。 FSDRは以下の要素で構成されます。 ディザスタ・リカバリ保護グループ(DR保護グループ) DR保護グループは、フル・スタック・アプリケーションのすべてのコンポーネントをグループ化し、すべてのコンポーネントをまとめてリカバリしてフル・スタック・アプリケーションをリストアできるようにします。 DR保護グループに追加できるコンポーネントは、FSDRが対応しているコンピュート・インスタンス、ボリューム・グループ、Oracle DatabaseなどのOCIリソースです。 これらのリソースは、DR保護グループのメンバーと呼ばれます。 DR保護グループは、プライマリ・リージョン(本番環境)とスタンバイ・リージョン(災害対策環境)にそれぞれ1つずつ作成し、それらは関連付けされます。 ディザスタ・リカバリ計画(DR計画) DR計画は、FSDRによって作成される自動DRワークフローで、プライマリDR保護グループ内のすべてのリソースの障害回復を実行します。 DR計画は、以下の4種類の計画タイプが存在します。 フェイルオーバー(計画外) スタンバイDR保護グループへのサービスの計画外切り替えを実行するDR計画のタイプ。 フェイルオーバー計画は、プライマリ・リージョンでのサービス停止をせず、スタンバイ・リージョンでアプリケーション・スタックを起動することで即時切り替えを実行します。 フェイルオーバー計画は通常、プライマリ・リージョンが停止または障害発生した場合に使用されます。 スイッチオーバー(計画済) プライマリDR保護グループからスタンバイDR保護グループへのサービスの計画済切り替えを実行するDR計画のタイプ。 スイッチオーバー計画は、プライマリ・リージョンのアプリケーション・スタックを停止してから、スタンバイ・リージョンでそれを起動することで、順次切り替えを実行します。 スイッチオーバー計画は通常、計画済サイト・メンテナンス、ソフトウェアのパッチ適用、DRテストおよび検証の目的で使用されます。 ドリルの開始 DRドリルは、災害対策環境が正常に起動できるかどうかをチェックするために実行されます。 スタンバイDR保護グループに、スイッチオーバーまたはフェイルオーバーの実行時に起動される環境のレプリカが作成されます。 ドリルの停止 DRドリルを停止するDR計画のタイプです。これにより、ドリルの開始で前に作成した本番スタックのレプリカが削除されます。 本チュートリアルでは、FSDRを用いてOCIリソースをDR保護グループのメンバーに追加し、スイッチオーバー計画を発行して実行するまでの流れをご紹介します。 目次 1. 準備作業(バケットの作成) 2. DR保護グループの作成 3. DR保護グループの関連付け 4. OCIリソースをDR保護グループのメンバーに追加 5. DR計画の作成 6. DR計画の実行 前提条件 :...","categories": [],
        "tags": [],
        "url": "/ocitutorials/fsdr/fsdr101/",
        "teaser": "/ocitutorials/fsdr/fsdr101/fsdr_component.png"
      },{
        ...
        "excerpt":"0. 概要 本チュートリアルは、HPCクラスタの計算ノードに最適なベアメタルインスタンスの BM.Optimized3.36 / BM.HPC.E5.144 を クラスタ・ネットワーク でノード間接続する、HPCワークロードを実行するためのHPCクラスタを構築する際のベースとなるインフラストラクチャを、予め用意された Terraform スクリプトを活用して自動構築し、そのインターコネクト性能を検証します。 この自動構築は、 Terraform スクリプトを リソース・マネージャ に読み込ませて作成する スタック を使用する方法と、 Terraform 実行環境を用意して Terraform CLIを使用する方法から選択することが出来ます。 このチュートリアルで作成する環境は、ユーザ管理、ホスト名管理、ファイル共有ストレージ、プログラム開発環境、ジョブスケジューラ等、必要なソフトウェア環境をこの上に整備し、ご自身の要件に沿ったHPCクラスタを構築する際の基礎インフラストラクチャとして利用することが可能です。 なお、これらのクラスタ管理に必要なソフトウェアの導入までを自動化する HPCクラスタスタック も利用可能で、詳細は OCI HPCチュートリアル集 の HPCクラスタを構築する(スタティッククラスタ自動構築編) を参照してください。 本チュートリアルで作成するHPCクラスタ構築用の Terraform スクリプトは、そのひな型が GitHub のパブリックレポジトリから公開されており、適用すると以下の処理を行います。 VCN と関連するネットワークリソース作成 Bastionノード作成 計算ノード用 インスタンス構成 作成 クラスタ・ネットワーク と計算ノード作成 HPCクラスタ内のノード間SSHアクセスに使用するSSH鍵ペア作成・配布 計算ノードの全ホスト名を記載したホストリストファイル作成 作成したBastionノード・計算ノードのホスト名・IPアドレス出力 Bastionノードは、接続するサブネットをパブリックとプライベートから選択することが可能（※1）で、以下のBastionノードへのログイン方法に合わせて選択します。 インターネット経由ログイン -&gt;...","categories": [],
        "tags": [],
        ...
        "excerpt":"0. 概要 オープンソースのMPI実装である OpenMPI は、 Modular Component Architecture （以降 MCA と呼称します。）を採用することで、ビルド時に組み込むコンポーネントを介して集合通信を含む多彩な機能を提供し、この MCA パラメータにはMPI集合通信性能に影響するものがあります。 特にMPI集合通信の高速化を意識して開発されている HCOLL や Unified Collective Communication （以降 UCC と呼称します。）は、その特性を理解して適切に利用することで、MPI集合通信性能を大幅に向上させることが可能です。 また OpenMPI は、高帯域・低遅延のMPIプロセス間通信を実現するためにその通信フレームワークに UCX を採用し、この UCX のパラメータにもMPI集合通信性能に影響するパラメータが存在します。 またMPI集合通信は、ノード内並列では実質的にメモリコピーとなるため、メモリ性能に影響するMPIプロセスのコア割当てや NUMA nodes per socket （以降 NPS と呼称します。）もその性能に影響します。 以上を踏まえて本パフォーマンス関連Tipsは、HPCワークロード向けベア・メタル・シェイプ BM.Optimized3.36 に於ける OpenMPI のMPI集合通信性能にフォーカスし、以下の 計測条件 を組合せたテストケース毎に以下の 実行時パラメータ を変えてその性能を Intel MPI Benchmarks で計測し、最適な...","categories": [],
        "tags": [],
        ...
        "excerpt":"0. 概要 PAPI は、異なるプラットフォーム間を共通のインターフェースで利用できるように設計された性能解析ツールで、プロファイリング対象のアプリケーションからAPIをコールすることで、CPU/GPUをはじめとする様々なハードウェアからプロファイリングに有益な情報を収集します。 この PAPI の可搬性は、ハードウェア固有の部分を吸収する下層と、プロファイリングを行うアプリケーション開発者が利用する抽象化された上位層にソフトウェアスタックを分割することで、これを実現しています。これらの関係は、 ここ に記載のアーキテクチャ図が参考になります。 PAPI のAPIは、HPCアプリケーションをプロファイリングするユースケースの場合、 Low Level API と High Level API から選択して利用することが出来ます。 High Level API は、内部で Low Level API を使用することでより高機能のプロファイリングが可能なAPIを提供し、 Low Level API に対して以下の利点があります。 より少ないAPIコールで同じプロファイリング機能を実現（※1） コードを修正することなく利用するハードウェアカウンタの変更が可能（※2） プロファイリング情報の出力制御が不要（※3） ※1）最も少ないケースは、プログラム中の計測範囲の前後に2回APIコールを挟み込むだけです。 ※2）アプリケーション実行時の環境変数で利用するハードウェアカウンタを指定します。これに対し Low Level API は、APIをコールして利用するハードウェアカウンタを指定するため、これを変更する場合再コンパイルが必要です。 ※3）計測範囲の最後を指定するAPIがコールされると、環境変数に指定したディレクトリにプロファイリング情報を格納したファイルが自動的に作成されます。これに対し Low Level API は、明示的に取得したハードウェアカウンタの値を出力する必要があります。 以上を踏まえて本プロファイリング関連Tipsでは、 High Level API を使用するサンプルプログラムを使って...","categories": [],
        "tags": [],
        "url": "/ocitutorials/hpc/benchmark/papi-profiling/",
        "teaser": null
      },{
        "title": "Score-P・Scalasca・CubeGUIで並列アプリケーションをプロファイリング",
        "excerpt":"0. 概要 Score-P は、HPCに於ける高並列アプリケーションをスケーラブルで簡易にプロファイリング（※1）することを目的に開発されたオープンソースのプロファイリングツールで、以下の代表的な並列プログラミングモデルに対応しています。 スレッド並列 OpenMP Pthreads プロセス並列 MPI SHMEM GPU並列 CUDA OpenCL OpenACC また Score-P は、並列アプリケーションの評価指標に対して、統計的に扱うプロファイリング手法（※1）とタイムスタンプと共に扱うトレーシング手法（※1）の何れにも対応しており、解析用途に合わせたプロファイリングが可能です。 ※1）本プロファイリング関連Tipsでは、性能向上を目的とした並列アプリケーションの性能解析プロセス全般を プロファイリング と呼称し、この中で評価指標を統計的に扱う プロファイリング の1つの方法を プロファイリング手法 と呼称します。これに対し、評価指標をタイムスタンプと共に扱う プロファイリング の1つの方法を トレーシング手法 と呼称します。 また Score-P は、 PAPI が取得するハードウェアカウンタ等の情報を取り込むことで、以下のように浮動小数点演算数等の情報をサブルーチンや関数単位で集計することが可能です。 Scalasca は、 Score-P が出力するトレーシング手法のデータをタイムスタンプ情報を元に解析する機能を持ち、並列アプリケーションのクリティカルパスの特定やプロセス間・スレッド間で発生する待ち時間等、プロファイリング手法では得られない情報を提供します。 また Scalasca は、 Score-P のフロントエンドとしても機能し、殆どの Score-P の操作を Scalasca から実施することが可能です。 CubeGUI は、Score-P が出力するプロファイリング手法のデータを読み込むことで、プロファイリング対象の並列アプリケーションを以下の3評価軸で表示し、 評価指標軸...","categories": [],
        "tags": [],
        "url": "/ocitutorials/hpc/benchmark/scorep-profiling/",
        "teaser": null
      },{
        "title": "プロファイリング情報に基づく並列アプリケーションチューニング方法",
        "excerpt":"0. 概要 並列アプリケーションのチューニングは、以下のステップを経て行われることが一般的です。 プロファイリング asis （※1）の所要時間計測 プロファイリング取得時の所要時間計測 両者に差がある場合プロファイリングオーバーヘッドの原因を特定 プロファイリングオーバーヘッド対象個所を除外してプロファイリング取得 プロファイリング情報から ホットスポット （※2）を特定 チューニング ホットスポット に対するチューニング手法検討 チューニング適用時のプロファイリング取得 プロファイリング情報から ホットスポット に対するチューニングの効果を確認 チューニング適用時の所要時間計測 asis とチューニング適用時の所要時間比較・チューニング効果確認 ※1）本パフォーマンス・プロファイリング関連Tipsでは、チューニング適用前のアプリケーションの状態を asis と呼称します。 ※2）本パフォーマンス・プロファイリング関連Tipsでは、所要時間のうち上位を占めるプログラム単位（サブルーチン・関数、MPI通信関数、IO等）を ホットスポット と呼称します。 以上のステップでチューニングの効果が得られた場合は、チューニング適用状態を asis として上記ステップを繰り返し、更なる性能向上を図ります。 以上を踏まえて本パフォーマンス・プロファイリング関連Tipsは、プロファイリングツールに以下のオープンソースソフトウェアを使用し、 Score-P Scalasca CubeGUI OpenMPI を使用する並列アプリケーションを ホットスポット となるMPI通信関数にフォーカスしてチューニングし、その性能を向上させる手順を解説します。 本手順は、 クラスタ・ネットワーク で相互接続する BM.Optimized3.36 を計算ノードとするHPCクラスタ環境で実行することを前提とし、 NAS Parallel Benchmarks （以降 NPB と呼称します。）の...","categories": [],
        "tags": [],
        "url": "/ocitutorials/hpc/benchmark/profiling-tuning/",
        "teaser": null
      },{
        ...
        "excerpt":"本チュートリアルでは、Oracle Application Express（APEX）で作成したアプリケーションにアクセスする際にMFAを要求する手順を紹介します。   本チュートリアルを完了すると、Identity DomainのユーザーAPEXアプリケーションにアクセスした際にMFAによる認証を要求することできます。   所要時間 : 約10分   前提条件 :     OCIチュートリアルOCI IAM Identity DomainでAPEXで作成したアプリケーションに認証と認可をするを参考に、APEXアプリケーションとIdentity Domainsの認証連携ができていること。   注意 : ※チュートリアル内の画面ショットについてはOracle Cloud Infrastructureの現在のコンソール画面と異なっている場合があります。       1. MFAの設定   MFAの要素として電子メールのワンタイム・パスワードやモバイルアプリケーションによる認証どを設定することができますが、本手順では電子メールのワンタイム・パスワードを要求するように設定します。   ドメイン・メニューの「認証」を選択します。ファクタ部分の電子メールが無効になっている場合には「ファクタの有効化または無効化」を選択します。      ファクタの有効化または無効化画面にて「電子メール」をチェックONにし、「変更の保存」を選択します。他のMFAを有効化したい場合は、そのMFAをチェックをONにします。      次に、APEXサンプルアプリアクセス時にMFAを必須にするサインオンポリシーを定義するため、ドメインメニュー「ドメイン・ポリシー」を選択します。サインオン・ポリシー部分にて「サインオン・ポリシーの作成」を選択します。      ポリシーの作成画面にて、任意の名前を入力し、「サインオン・ポリシーの作成」を選択します。      作成されたサインオン・ポリシーの画面にてメニュー「サインオン・ルール」を選択し、「サインオン・ルールの追加」を選択します。         サインオン・ルールの追加画面にて、ルール名に任意の名前を入力し、アクション部分にて下記を設定し、「追加」を選択します。     追加ファクタの要求にチェック   指定されたファクタのみにチェック   電子メールににチェック   頻度‐セッションごと・・・・・にチェック   登録‐必須にチェック          サインオン・ルールが作成されたことを確認し、メニュー「アプリケーション」を選択し、「アプリケーションの追加」を選択します。     アプリケーションの追加画面にて上記にて作成したAPEX用機密アプリケーションにチェックし「アプリケーションの追加」を選択します。      右上のアクションより「サインオン・ポリシーのアクティブ化」を選択します。      確認画面でも「サインオン・ポリシーのアクティブ化」を選択します。     動作確認  アプリケーションにアクセスしMFAの動作確認をします。   アイデンティティ・ドメインのログイン画面が表示されたことを確認し、上記で作成したテストユーザーのID/パスワードを指定し「サインイン」を選択します。     セキュアな検証の有効化画面にて「セキュアな検証の有効化」を選択します。     MFA要素の選択画面にて「電子メール」を選択します。     テストユーザーのメールアドレスに記載されているパスコードを確認します。   確認したパスコードをブラウザ画面のコード部分に入力し、「電子メール・アドレスの確認」を選択します。     MFA要素「電子メール」の登録が完了したことを確認し「完了」を選択すると、APEXアプリケーションにアクセスできます。        次回以降からアプリケーションへのアクセスは、ログイン画面⇒MFA(電子メールワンタイムパスコード指定)⇒アプリケーションへという流れになることを確認します。         ","categories": [],
        "tags": ["intermediate"],
        "url": "/ocitutorials/identity/identitydomain-apex-mfa/",
        ...
        "excerpt":" ","categories": [],
        "tags": [],
        "url": "/ocitutorials/intermediates/fsdr/",
        "teaser": "/ocitutorials/fsdr/fsdr101/fsdr_component.png"
      },{
        ...
        "excerpt":"OCIのモニタリング・サービスではカスタム・メトリックの機能を利用して、通常のメトリックでは監視できないユーザ独自のメトリックをモニタリングで監視できます。カスタム・メトリックで追加したメトリックも通常のメトリックと同様にアラームで事前定義した条件にメトリック値が合致した際には管理者に通知を行うことで管理者はタイムリーに適切な対処を行うことができます。 この章では、デフォルトのメトリックでは監視できないコンピュート・インスタンスのディスク使用率をMonitoringのカスタム・メトリックで監視する手順をご紹介します。ディスク使用率以外でもOSで稼働する特定プロセスの有無（プロセスが見つかれば1、なければ0をポスト）など数値で取得できるメトリックであればカスタム・メトリックで監視できます。 所要時間 : 約30分 前提条件1 : インスタンスを作成する - Oracle Cloud Infrastructureを使ってみよう(その3) を通じてコンピュート・インスタンスの作成が完了していること 注意 : チュートリアル内の画面ショットについては Oracle Cloud Infrastructure の現在のコンソール画面と異なっている場合があります。 前提条件2 : モニタリング機能でOCIのリソースを監視するを通じてモニタリングのメトリックが参照できること 必要な権限 : カスタム・メトリックの監視には以下の権限設定が最低限必要です。 動的グループ instance.compartment.id = '&lt;compartment_ocid&gt;' 動的グループの概要と設定方法については以下を参照ください。 参考：OCI活用資料集：IDおよびアクセス管理 (IAM) 詳細 ポリシー Allow dynamic-group &lt;group-name&gt; to use metrics in tenancy 1. Pythonスクリプトの作成 カスタム・メトリックはOCI CLIやAPIで設定することができますが、ここではPython スクリプトでカスタム・メトリックを監視する方法を紹介します。 まだPythonがインストールされていない場合は、インストールしておきます。 今回はコンピュートのディスク使用率を取得して、モニタリングにポストするPythonスクリプトを作成します。...","categories": [],
        "tags": ["management"],
        "url": "/ocitutorials/management/monitoring_custom_metric/",
        "teaser": null
      },{
        ...
        "excerpt":"チュートリアル概要 : OCIのMonitoring・サービスでもカスタム・メトリックを作成することができますが、Stack Monitoringのメトリック拡張を利用することで面倒なスクリプト作成をせずにGUIベースで簡単にユーザ独自のメトリック監視を設定できます。 このチュートリアルでは、Stack Monitoring のメトリック拡張を設定する手順をご紹介します。 所要時間 : 約30分 前提条件1 : テナンシ上で以下のリソースが作成済であること コンパートメント ユーザー ユーザーグループ VCN 対象リソース 前提条件2 : Stack Monitoring が有効化済みであること こちらを参考にStack Monitoringを有効化できます。 前提条件3 : Stack Monitoringで対象リソースが監視されていること こちらを参考にStack Monitoring でBaseDB を監視できます。 こちらを参考にStack Monitoring でCompute を監視できます。 1.メトリック拡張の作成 本チュートリアルでは例としてデータベースに登録されているユーザー数を取得するメトリックを追加します。 「メトリック拡張の作成」をクリックします。 メトリック拡張のプロパティの項目は以下のように設定します。 名前：ME_任意の名前 表示名：任意の表示名 収集方法のプロパティは以下のように設定します。 リソース・タイプ：コンテナDB 収集方法：SQL 収集頻度：任意の時間 SQL問合せ：SELECT COUNT(*) from...","categories": [],
        "tags": ["management"],
        ...
        "excerpt":"チュートリアル概要 : 本チュートリアルでは、Stack Monitoring のプロセスベースのカスタム・リソースを設定しホストで実行されているプロセスを監視する手順をご紹介します。プロセスベースのカスタム・リソースを利用することで、プロセスが使用しているCPUやメモリーの使用率を監視できます。 所要時間 : 約20分 前提条件1 : テナンシ上で以下のリソースが作成済であること コンパートメント ユーザー ユーザーグループ VCN 対象リソース 前提条件2 : Stack Monitoring が有効化済みであること こちらを参考にStack Monitoringを有効化できます。 前提条件3 : Stack Monitoringで対象リソースが監視されていること こちらを参考にStack Monitoring でBaseDB を監視できます。 こちらを参考にStack Monitoring でCompute を監視できます。 1.プロセス・セットの作成 本チュートリアルではコンピュートで実行されているnginxのプロセスを監視するための設定をご紹介します。 プロセスを監視するためのプロセス・セットは、プロセスのモニターから作成できます。 プロセスセットの作成をクリックします。 プロセス・セットのプロパティは以下のように設定します。 表示名：任意の名前 ラベル：nginx-main-process プロセス・コマンド：nginx プロセス・ライン行正規表現パターン：nginx: master.* ターゲット・ホスト：プロセスを実行している監視対象のリソース リソースタイプと収集方法について プロセス・ユーザーはプロセスを実行中のユーザーの名前がある場合は指定します。 プロセス・セットのプロパティを設定したら、「作成してマップ」クリックします。 プロセスセットを設定したリソースのトポロジから作成したプロセスセットをクリックします。...","categories": [],
        "tags": ["management"],
        ...
        "teaser": "/ocitutorials/security/networkfirewall-setup/nfw32.png"
      },{
        ...
        "teaser": "/ocitutorials/security/networkfirewall-sslinspect-fwd/sslfwd20.png"
      },{
        ...
        "teaser": "/ocitutorials/security/networkfirewall-sslinspect-inb/sslinb25.png"
      },{
        ...
        "excerpt":"Oracle Data Safeは、Oracle Databaseのセキュリティを強化・補完するためのクラウドサービスです。 Data Safeを有効化すると以下の機能を利用することができます。 セキュリティ・アセスメント ユーザー・アセスメント 機密データ検出 データ・マスキング アクティビティ監査 SQL Firewall（DBのバージョンが23aiのみ対応） 本手順ではBaseDBに対してData Safeを設定する手順をご紹介しますが、ExaDB-DやAutomomous Databaseも同様の手順で設定できます。 所要時間 : 約30分 前提条件 : OCIのBase DBが作成済み Data SafeにBase DBを登録する ユーザの作成 Data Safeに登録する際にPDBのデータベース・ユーザが必要です。 ユーザーの名前は自由ですが、OCIコンソールでの登録画面では DATASAFE$ADMIN ユーザー名がデフォルトでセットされてるため、この名前を使用して作成します。パスワードは14文字以上で、大文字、小文字、数字および特殊文字をそれぞれ1つ以上含める必要があります。 PDBにログインし、以下のSQLを実行します。 CREATE USER DATASAFE$ADMIN identified by \"パスワード\"; GRANT CONNECT, RESOURCE TO DATASAFE$ADMIN; BaseDBの場合はOracleが提供しているスクリプトを実行することで、Data Safeの機能を利用するためのロールを付与または取り消しができます。このスクリプトはOCIコンソールの [メニューバー] → [Oracle Database]...","categories": [],
        "tags": [],
        "url": "/ocitutorials/security/datasafe-basedb/",
        "teaser": null
      },{
        "title": "Oracle Data Safe チュートリアルまとめ",
