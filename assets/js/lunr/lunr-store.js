var store = [{
        ...
        "excerpt":"このチュートリアルは、HPC/GPUクラスタの計算/GPUノードに最適なベアメタルインスタンス（本チュートリアルではHPCクラスタ向けに BM.Optimized3.36 GPUクラスタ向けに BM.GPU4.8 を使用）を クラスタ・ネットワーク でノード間接続する、HPC/機械学習ワークロードを実行するためのHPC/GPUクラスタを構築する際のベースとなるインフラストラクチャを、予め用意された Terraform スクリプトを活用して自動構築し、そのインターコネクト性能を検証します。 この自動構築は、Terraformスクリプトを リソース・マネージャ に読み込ませて作成する スタック を使用する方法と、Terraform実行環境を用意してTerraform CLIを使用する方法から選択することが出来ます。 このチュートリアルで作成する環境は、ユーザ管理、ホスト名管理、共有ファイルシステム、プログラム開発環境、コンテナランタイム、ジョブスケジューラ等、必要なソフトウェア環境をこの上に整備し、ご自身の要件に沿ったHPC/GPUクラスタを構築する際の基礎インフラストラクチャとして利用することが可能です。 なお、これらのクラスタ管理に必要なソフトウェアの導入までを自動化する HPCクラスタスタック も利用可能で、詳細は HPCクラスタを構築する(スタティッククラスタ自動構築編) や GPUクラスタを構築する(スタティッククラスタ自動構築編) を参照ください。 本チュートリアルで作成するHPC/GPUクラスタ構築用のTerraformスクリプトは、そのひな型がGitHubのパブリックレポジトリから公開されており、適用すると以下の処理を行います。 VCNと関連するネットワークリソース構築 Bastionノード構築 計算/GPUノード用 インスタンス構成 作成 クラスタ・ネットワークと計算/GPUノード構築 HPC/GPUクラスタ内のノード間SSHアクセスに使用するSSH鍵ペア作成・配布 計算/GPUノードの全ホスト名を記載したホストリストファイル（/home/opc/hostlist.txt）作成 構築したBastionノード・計算/GPUノードのホスト名・IPアドレス出力 Bastionノード構築は、 cloud-init 設定ファイル(cloud-config)を含み、cloud-initがBastionノードデプロイ時に以下の処理を行います。 タイムゾーンをJSTに変更 ホームディレクトリ領域のNFSエクスポート 計算/GPUノードのDNS名前解決をショートホスト名で行うための resolv.conf 修正 また計算/GPUノード用インスタンス構成は、cloud-configを含み、cloud-initが計算/GPUノードデプロイ時に以下の処理を行います。 タイムゾーンをJSTに変更 NVMe SSDローカルディスク領域ファイルシステム作成 firewalld停止 ルートファイルシステム拡張 クラスタ・ネットワーク接続用ネットワークインターフェース作成 BastionノードのDNS名前解決をショートホスト名で行うための resolv.conf...","categories": [],
        "tags": [],
        ...
        "excerpt":"クラスタ・ネットワーク を使用するインスタンスは、接続に必要なソフトウェアがインストールされている必要がありますが、これらを含んだOSイメージが マーケットプレイス から提供されています。 本テクニカルTipsは、このOSイメージの適切な選び方を解説します。 0. 概要 マーケットプレイス から提供される クラスタ・ネットワーク 対応OSイメージは、ベースOSにOracle Linuxを採用し、シェイプがBM.HPC2.36/ BM.Optimized3.36 の場合そのバージョンは7.9と8から選択でき、 BM.GPU4.8/BM.GPU.GM4.8 の場合7.9のみ選択可能です。 なお、2023年6月29日時点のバージョン8系クラスタ・ネットワーク対応OSイメージの最新は、8.7です。 下表は、クラスタ・ネットワークに対応したシェイプと対応するOSイメージの組み合わせを示しています。 シェイプ OSイメージ名 ベースOS マーケットプレイスURL BM.HPC2.36 HPC Cluster Networking Image Oracle Linux 7.9 Link (*1)   HPC Cluster Networking Image Oracle Linux 8 Link (*2) BM.Optimized3.36 HPC Cluster Networking Image Oracle Linux 7.9...","categories": [],
        "tags": [],
