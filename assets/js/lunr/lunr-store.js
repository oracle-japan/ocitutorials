var store = [{
        ...
        "excerpt":"0. 概要 ファイル・ストレージ は、以下の特徴を持ち、高い可用性と運用性を求められるHPC/GPUクラスタ向けのファイル共有ストレージに最適なマネージドNFSサービスです。 マウント・ターゲット がHA構成となっていて高いサービス可用性を提供 ファイル・システム 内のデータが複製されていて高いデータ可用性を提供 マウント・ターゲット のアップグレードによる動的な性能向上が可能（※1） ヘッドノードの マウント・ターゲット のソフトウェアメンテナンスが不要 OCIコンソールからGUIでデプロイできるのため構築難易度が低い ※1）アップグレードで選択可能なHigh Performanceタイプの マウント・ターゲット の詳細は、 ここ を参照してください。 OCIは、マネージドブロックボリュームサービスの ブロック・ボリューム も提供しており、ベア・メタル・インスタンスと組み合わせることで ファイル・ストレージ よりもコストパフォーマンスの高いファイル共有ストレージを構築することが出来ますが、構築手順は圧倒的に ファイル・ストレージ が簡単です。 ファイル・ストレージ とブロック・ボリュームNFSサーバの比較詳細は、 OCI HPCテクニカルTips集 の HPC/GPUクラスタ向けファイル共有ストレージの最適な構築手法 を参照してください。 以上を踏まえて本チュートリアルは、 ファイル・ストレージ を使用するファイル共有ストレージを構築、ファイル共有クライアントとなる計算/GPUノードからこの領域をマウントするまでの手順を解説します。 所要時間 : 約30分 前提条件 : ファイル共有ストレージを収容するコンパートメント(ルート・コンパートメントでもOKです)の作成と、このコンパートメントに対する必要なリソース管理権限がユーザーに付与されていること。 注意 : 本コンテンツ内の画面ショットは、現在のOCIコンソール画面と異なっている場合があります。 1. 事前作業 1-0. 概要...","categories": [],
        "tags": [],
        ...
        "excerpt":"0. 概要 BM.DenseIO.E5.128 は、6.8 TBのNVMe SSDを12台ローカル接続し、ストレージに高いスループットとIOPSを要求する様々なワークロード用途に最適なベア・メタル・シェイプです。 また BM.DenseIO.E5.128 は、100 GbpsのTCP/IP接続用ポートを1個搭載し、NVMe SSDローカルディスクに構築するストレージ領域を外部にサービスする際、100 Gbpsの帯域を全てクライアントへのサービスに割当てることで、ストレージとのアクセスにiSCSI接続によるネットワーク帯域を消費する ブロック・ボリューム を使用する場合と比較し、高速なストレージサービスサーバとして利用することが出来ます。 これに対して BM.DenseIO.E5.128 のNVMe SSDローカルディスクを使用するストレージサービスは、 ブロック・ボリューム を使用する場合と比較し、以下の点を考慮する必要があります。 NVMe SSDローカルディスク上のデータ保護を自身で行う必要がある ベア・メタル・インスタンスの故障でNVMe SSDローカルディスク上のデータを消失するリスクがある データ格納容量が最大で38 TB程度（RAID10でファイルシステムを構成した場合）に限定され拡張出来ない 本チュートリアルは、これらの考慮点に対して以下のアプローチを取り、 ソフトウェアRAIDによるRAID10で最低限の可用性を確保 NVMe SSDローカルディスク上のファイルを定期的に他の長期保存ストレージにバックアップ（対象を短期保存データに限定） 2. のアプローチでバックアップの後ファイルを削除して空き容量を確保 以下構成の短期保存データ用高速ファイル共有ストレージを構築します。 NFSサーバ シェイプ ： BM.DenseIO.E5.128 OS ： Oracle Linux 9.4（Oracle-Linux-9.4-2024.07.31-0 UEK） ソフトウェアRAID・NFS ： OS付属のものを利用 RAID構成 RAIDレベル ： RAID10...","categories": [],
        "tags": [],
        ...
        "excerpt":"0. 概要 HPC/GPUクラスタと共に利用するファイル共有ストレージは、その構築手法を決定する際に以下の評価項目を考慮する必要があります。 計算/GPUノードの複数ノード同時アクセス時性能 ランニングコスト ストレージ領域に格納するデータの可用性 ファイル共有ストレージサービスの可用性 システム構築・運用のしやすさ 本テクニカルTipsは、HPC/GPUクラスタ向けにNFSでサービスするファイル共有ストレージを構築することを念頭に、いくつかのファイル共有ストレージ構築手法を紹介し、上記評価基準を元にどの手法を採用すればよいか、その考慮点を解説します。 NFSでサービスするファイル共有ストレージを構築する際の最初の考慮点は、マネージドのNFSサービスである ファイル・ストレージ を使用する方法と、自身で ベア・メタル・インスタンス からNFSサーバを構築する（以降”ベア・メタル・インスタンスNFSサーバ”と呼称）方法の、どちらを採用するかです。 ここで ファイル・ストレージ を採用する場合は、性能要件に合わせて マウント・ターゲット に標準タイプ（以降”標準FSS”と呼称）とHigh Performanceタイプ（以降”高性能FSS”と呼称）（※1）のどちらを使用するかを考慮します。 ※1）High Performanceタイプの マウント・ターゲット は、その性能違いで3種類存在しますが、本テクニカルTipsでは読み取りスループット設定が80 Gbpsで性能が最も高い HPMT-80 を前提に解説します。 High Performanceタイプの マウント・ターゲット 詳細は、 ここ を参照して下さい。 またベア・メタル・インスタンスNFSサーバを採用する場合は、以下2種類のどちらにするかを考慮します。 ストレージ領域に ブロック・ボリューム を使用しこれをベア・メタル・インスタンスにアタッチする方法（以降”ブロック・ボリュームNFSサーバ”と呼称） ベア・メタル・インスタンス にNVMe SSDドライブを搭載するDenceIOシェイプを使用する方法（以降”DenceIO NFSサーバ”と呼称） 以上より、本テクニカルTipsで解説するファイル共有ストレージは、以下4種類です。 構築手法の呼称 使用するサービス ヘッドノード ストレージ領域 保存対象 データ （※2） ターゲット...","categories": [],
        "tags": [],
