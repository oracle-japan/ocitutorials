var store = [{
        ...
        "excerpt":"クラスタ・ネットワーク を使用するインスタンスは、接続に必要なソフトウェアがインストールされている必要がありますが、これらを含んだOSイメージが クラスタネットワーキングイメージ として マーケットプレイス から提供されています。 本テクニカルTipsは、この クラスタネットワーキングイメージ の適切な選び方を解説します。 0. 概要 クラスタネットワーキングイメージ は、ベースOSに Oracle Linux を採用し、 クラスタ・ネットワーク への接続に必要な以下のソフトウェアが予めインストールされています。 Mellanox OFED クラスタ・ネットワーク にインスタンスを接続するNIC（NVIDIA Mellanox ConnectX）のドライバーソフトウェアです。 wpa_supplicant インスタンスが クラスタ・ネットワーク に接続する際行われる802.1X認証（※1）のクライアントソフトウェアです。 802.1X認証関連ユーティリティソフトウェア インスタンスが クラスタ・ネットワーク に接続する際行われる802.1X認証に必要な機能を提供するユーティリティソフトウェアです。 クラスタ・ネットワーク 設定ユーティリティソフトウェア クラスタ・ネットワーク 接続用ネットワークインターフェースのIPアドレス設定等の機能を提供するユーティリティソフトウェアです。 ※1）802.1X認証の仕組みは、 ここ のサイトが参考になります。 また クラスタネットワーキングイメージ は、以下の観点で異なる用途のものが用意されています。 802.1X認証関連ユーティリティソフトウェアと クラスタ・ネットワーク 設定ユーティリティソフトウェアの提供方法 Oracle Cloud Agent （以降 OCA...","categories": [],
        "tags": [],
        "url": "/ocitutorials/hpc/tech-knowhow/osimage-for-cluster/",
        "teaser": null
      },{
        "title": "クラスタ・ネットワーク非対応OSイメージを使ったクラスタ・ネットワーク接続方法",
        "excerpt":"クラスタ・ネットワーク に接続するインスタンスは、接続に必要なソフトウェアがインストールされている必要があり、これらを含んだOSイメージが クラスタネットワーキングイメージ として マーケットプレース から提供されていますが、これらのベースとなるOSは、Oracle Linuxのみです。 本テクニカルTipsは、 クラスタ・ネットワーク 対応シェイプの BM.Optimized3.36 や BM.GPU4.8/BM.GPU.A100-v2.8 を使用するインスタンスを Oracle Linux 以外のOSで クラスタ・ネットワーク に接続する方法を解説します。 注意 : 本コンテンツ内の画面ショットは、現在のOCIコンソール画面と異なっている場合があります。 0. 概要 クラスタ・ネットワーク に接続するインスタンスは、以下の条件を満たす必要があります。 クラスタ・ネットワーク 対応シェイプ（ ここ を参照）を使用している クラスタ・ネットワーク のデプロイに伴ってデプロイされている クラスタ・ネットワーク 接続のための以下ソフトウェアがインストールされている Mellanox OFED wpa_supplicant（※1） oci-cn-auth（※2） クラスタ・ネットワーク 接続用ネットワークインターフェースがOS上で構築されている ※1）クラスタ・ネットワーク は、インスタンスが接続する際802.1X認証を要求しますが、この処理を行うクライアントソフトウェアがwpa_supplicantです。802.1X認証の仕組みは、以下のサイトが参考になります。 https://www.infraexpert.com/study/wireless14.html ※2）クラスタ・ネットワーク に接続する際の802.1X認証で必要な認証処理機能を提供するユーティリティーソフトウェアで、GitHubから公開されています。 これらの条件の中で、 クラスタ・ネットワーク 接続のためのソフトウェアは、ベースOSに Oracle Linux...","categories": [],
        "tags": [],
