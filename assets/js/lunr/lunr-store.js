var store = [{
        ...
        "excerpt":"マネージドKubernetesサービスであるOralce Container Engine for Kubernetes(OKE)を中心とした、コンテナ・ネイティブなサービス群です。 Oracle Container Engine for Kubernetes（以下OKE）は、OracleのマネージドKubernetesサービスです。この共通手順では、OCIやOKEを操作するためCLI実行環境の構築（Resource Managerを使用）と、OKEを使ってKubernetesクラスターをプロビジョニングするまでの手順を記します。 前提条件 クラウド環境 Oracle Cloudのアカウントを取得済みであること ハンズオン環境のイメージ 1.ポリシーの設定 はじめにポリシーの設定をします。OKEクラスターをプロビジョニングするためにはOCIにポリシーと呼ばれる権限を設定する必要があります。ポリシーはテナントまたはコンパートメントに対して設定できます。 テナントについて Oracle Cloud Infrastructure には、テナントの内部を仕切る「コンパートメント」という区画を作成し、その中にリソースを配置したり、コンパートメントに「ポリシー」を割り当てることを通じて権限を設定したりすることができます。 詳細はこちらをご確認ください。 OCIコンソールにブラウザからアクセスします。お持ちのクラウドアカウントを使ってサインインします。 1．クラウドアカウント名（テナント名）を入力し、「Next」をクリックします 2．Identity Providersで「oracleidentitycloudservice」を選択します 3．「Continue」をクリックします 4．クラウドユーザー名とパスワードを入力して「サイン・イン」をクリックします ログイン後はじめにご確認ください!! 右上の人型アイコンをクリックして、直下に表示されるユーザー名が以下のようになっているか確認してください。 ログインできたら、OCIコンソール画面左上のハンバーガーメニューを展開し、Identity &amp; Security⇒ポリシーを選択します。 任意のコンパートメントを選択し、ポリシーの作成ボタンをクリックします。 手動エディタの表示をオンにします。 下記項目を入力して、「作成」をクリックします。 項目 入力値 名前 OKE_Policy 説明 OKE_Policy ポリシー・ビルダー トライアル環境もしくはOracle Cloud契約済の環境で管理者権限をお持ちの方(Administratorグループまたはそれに準ずるグループにお使いのユーザが含まれている方) Allow service OKE...","categories": [],
        "tags": [],
