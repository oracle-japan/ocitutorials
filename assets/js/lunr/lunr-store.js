var store = [{
        ...
        "title": "HPCクラスタを構築する",
        "excerpt":"Oracle Cloud Infrastructure（以降OCIと記載）は、以下の特徴を持ち、HPCワークロードを実行するHPCクラスタにとって最適なクラウトサービスです。 仮想化オーバーヘッドの無いHPC用途に特化したベアメタルシェイプ RoCEv2を使用する高帯域・低レイテンシRDMAインターコネクトネットワーク このチュートリアルは、OCIのマーケットプレイスから利用可能なHPCクラスタ構築のためのリソース・マネージャ用スタックを利用し、以下構成の典型的なHPCクラスタを構築、そのインターコネクト性能を検証します。 HPC向けIntel Ice Lakeプロセッサ搭載計算ノード（BM.Optimized3.36） シェイプ詳細は以下URLを参照 https://docs.oracle.com/ja-jp/iaas/Content/Compute/References/computeshapes.htm#baremetalshapes__bm-optimized 100 Gbps RoCEv2 RDMAインターコネクト インターネットからSSH接続可能なbastionノード OS: Oracle Linux 7.9 ジョブスケジューラ: Slurm HPCクラスタ内ホームディレクトリNFS共有 このHPCクラスタ構築用スタックを利用すると、通常であれば数日かかるようなHPCクラスタ構築作業を、OCIコンソールのGUIから10項目程度のメニューを選択するだけで実施することが可能になります。 リソース・マネージャについては、以下のチュートリアルも参考にしてください。 https://oracle-japan.github.io/ocitutorials/intermediates/resource-manager/ 所要時間 : 約30分 前提条件 : HPCクラスタを収容するコンパートメント(ルート・コンパートメントでもOKです)の作成と、このコンパートメントに対する必要なリソース管理権限がユーザーに付与されていること。 注意 : チュートリアル内の画面ショットについては、OCIの現在のコンソール画面と異なっている場合があります。 1. スタックの作成 リソース・マネージャでリソースをデプロイする場合、まずそのためのスタックを作成する必要があります。 本章は、マーケットプレースから提供するHPCクラスタ構築用スタックを元に、前述のHPCクラスタ環境を構築するためのスタックを作成します。 以下マーケット・プレースのHPCクラスタ構築用スタックページにアクセスします。 https://cloud.oracle.com/marketplace/application/67628143/ OCIコンソールへのログイン画面が表示された場合（まだログインしていない場合）、ログインを完了します。 表示される以下画面の右上で、コンパートメント をHPCクラスタを構築するコンパートメントに指定、使用許諾 チェックボックスをチェックし、 スタックの起動 ボタンをクリックします。 表示される以下 スタック情報 画面で、以下の情報を入力し、下部の...","categories": [],
        "tags": [],
